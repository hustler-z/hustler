...
#
# ./sdk_monitor.py --port /dev/ttyS3 --baud 115200 --eol CR baremetal.elf
    ...
    def run(self):
        ...
        try:
            while self.alive:
                ...
                if len(data):
                    #self.event_queue.put((TAG_SERIAL, b'phytium:/$'), False)
...
class Monitor(object):
    ...
    def __init__(self, serial_instance, elf_file, print_filter, make='make', encrypted=False,
        ...
        # internal state
        self._last_line_part = b'' #b'phytium:/$ '
        self._gdb_buffer = b''
    ...
    # def print_sh_header(self):
    #     # show title
    #     # self.event_queue.put((TAG_KEY, str('')))
    #     # self.event_queue.put((TAG_SERIAL, b'\r'), False)
    #     # self.event_queue.put((TAG_SERIAL, b'\n'), False)
    #     # self.event_queue.put((TAG_SERIAL, self._last_line_part), False)
    #     # self.event_queue.put((TAG_SERIAL_FLUSH, b''), False)
    #     return
...
def main():
    ...
    if os.name == 'nt' and args.port.startswith('COM'): # 是Windows系统
        args.port = args.port.replace('COM', r'\\.\COM')
        yellow_print('--- WARNING: GDB cannot open serial ports accessed as COMx')
        yellow_print('--- Using %s instead...' % args.port)
    elif args.port.startswith('/dev/tty.') and sys.platform == 'darwin': # 是UNIX类系统
    ...
    try:
        ...
        monitor.elfshheader = bytes(args.elfshheader, encoding="utf8")

        yellow_print('--- Phytium SDK Monitor on {p.name} {p.baudrate} ---'.format(
            p=serial_instance))
