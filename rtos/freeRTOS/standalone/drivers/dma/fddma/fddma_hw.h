...
#define FDDMA_CHAN_OFFSET(channel_id)                 (0x40 * (channel_id))
#define FDDMA_CHAN_DDR_UP_ADDR_OFFSET(channel_id)     (0x40 + FDDMA_CHAN_OFFSET(channel_id)) /* 内存中源/目的地址高32位 */
#define FDDMA_CHAN_DDR_LOW_ADDR_OFFSET(channel_id)    (0x44 + FDDMA_CHAN_OFFSET(channel_id)) /* 内存中源/目的地址低32位 */
#define FDDMA_CHAN_DEV_ADDR_OFFSET(channel_id)        (0x48 + FDDMA_CHAN_OFFSET(channel_id)) /* 设备中源/目的地址32位 */
#define FDDMA_CHAN_TS_OFFSET(channel_id)              (0x4C + FDDMA_CHAN_OFFSET(channel_id)) /* 需要传输的总数据字节数 */
#define FDDMA_CHAN_CRT_UP_ADDR_OFFSET(channel_id)     (0x50 + FDDMA_CHAN_OFFSET(channel_id)) /* 当前内存需要读写数据的高32位 */
#define FDDMA_CHAN_CRT_LOW_ADDR_OFFSET(channel_id)    (0x54 + FDDMA_CHAN_OFFSET(channel_id)) /* 当前内存需要读写数据的低32位 */
#define FDDMA_CHAN_CTL_OFFSET(channel_id)             (0x58 + FDDMA_CHAN_OFFSET(channel_id)) /* 通道控制寄存器 */
#define FDDMA_CHAN_STS_OFFSET(channel_id)             (0x5C + FDDMA_CHAN_OFFSET(channel_id)) /* 通道状态寄存器 */
#define FDDMA_CHAN_TIMEOUT_CNT_OFFSET(channel_id)     (0x60 + FDDMA_CHAN_OFFSET(channel_id)) /* 超时等待阈值 */

/*register for dma bdl*/
#define FDDMA_CHAN_CTRL(channel_id)                   (0x58 + FDDMA_CHAN_OFFSET(channel_id)) /* 通道使能寄存器 */
#define FDDMA_CHAN_DSIZE(channel_id)                  (0x64 + FDDMA_CHAN_OFFSET(channel_id)) /* AXI通道每次读或写传输数据长度 */
#define FDDMA_CHAN_DLENTH(channel_id)                 (0x68 + FDDMA_CHAN_OFFSET(channel_id)) /* AXI通道每次读或写传输BURST数据长度 */
#define FDDMA_CHAN_BUFFER_SIZE(channel_id)            (0x54 + FDDMA_CHAN_OFFSET(channel_id)) /* 当前整个BDL条条目列表对应的数据总长度，所有BDL的总数据 */
#define FDDMA_CHAN_BDL_VALID_NUM(channel_id)          (0x4c + FDDMA_CHAN_OFFSET(channel_id)) /* 当前BDL条目中的有效条目数量，有效条目数量 = 填入寄存器的数值 + 1 */

#define FDDMA_CHAN_AXI_READ_SIZE_SET(x) SET_REG32_BITS((x), 3, 2)    /* AXI每次读传输数据长度，00:4Byte,01:1Byte,02:2Byte */
#define FDDMA_CHAN_AXI_WRITE_SIZE_SET(x) SET_REG32_BITS((x), 1, 0)   /* AXI每次写传输数据长度，00:4Byte,01:1Byte,02:2Byte */
#define FDDMA_CHAN_AXI_READ_LENGTH_SET(x) SET_REG32_BITS((x), 15, 8) /* AXI每次写传输数据长度，00:1Byte,01:2Byte,03:4Byte */
#define FDDMA_CHAN_AXI_WRITE_LENGTH_SET(x) SET_REG32_BITS((x), 7, 0) /* AXI每次写传输数据长度，00:1Byte,01:2Byte,03:4Byte */

#define FDDMA_CHAN_DATA(channel_id)                    (0x6c + FDDMA_CHAN_OFFSET(channel_id))
 ...
 */
#define FDDMA_CHAN_0_3_SEL_EN(channel_id)            BIT((channel_id) * 8 + 7)      /* 1: 通道chan信号源选择有效 */
#define FDDMA_CHAN_0_3_SEL(channel_id, sel)          ((sel) <<  ((channel_id) * 8)) /* 通道chan信号源选择sel项 */
#define FDDMA_CHAN_0_3_SEL_MASK(channel_id)          (GENMASK(7, 0) << ((channel_id) * 8))
#define FDDMA_CHAN_0_3_SEL_BDL(channel_id)           BIT((channel_id)  * 8)

/** @name FDDMA_STA_OFFSET Register
 */
#define FDDMA_STA_CHAN_REQ_DONE(channel_id)          BIT((channel_id) * 4) /* 通道chan的block请求完成时置1，写1后清0 */
#define FDDMA_BDL_STA_CHAN_REQ_DONE(channel_id)      BIT((channel_id) * 8) /* BDL模式通道chan的block请求完成时置1，写1后清0 */

/** @name FDDMA_MASK_INTR_OFFSET Register
 */
#define FDDMA_MASK_EN_GLOBAL_INTR                     BIT(31)         /* 全局中断使能输出控制位 */
#define FDDMA_MASK_EN_CHAN_INTR(channel_id)           BIT(channel_id) /* 通道chan的中断输出控制位 */

/** @name FDDMA_CHAN_BIND_OFFSET Register
 */
#define FDDMA_CHAN_BIND(channel_id)                   BIT(channel_id) /* 1: 通道已绑定了外设DMA请求信号线 */

/** @name FDDMA_CHAN_4_7_CFG_OFFSET Register
 */
#define FDDMA_CHAN_4_7_SEL_EN(channel_id)             BIT(((channel_id) - 4) * 8 + 7)      /* 1: 通道chan信号源选择有效 */
#define FDDMA_CHAN_4_7_SEL(channel_id, sel)           ((sel) <<  (((channel_id) - 4) * 8)) /* 通道chan信号源选择sel项 */
#define FDDMA_CHAN_4_7_SEL_MASK(channel_id)           (GENMASK(7, 0) << (((channel_id) - 4) * 8))
 ...
 */
static inline u32 FDdmaReadChanStatus(uintptr base_addr, u32 channel_id)
{
    return FDdmaReadReg(base_addr, FDDMA_CHAN_STS_OFFSET(channel_id));
}
/************************** Function Prototypes ******************************/
/* 复位DDMA控制器 */
...
/* 设置通道超时 */
void FDdmaSetChanTimeout(uintptr base_addr, u32 channel_id, u32 timeout);
