...
FError FGmacLwipPortInit(FGmacLwipPort * instance_p)
{
    ...
    if(instance_p->feature & FGMAC_LWIP_PORT_CONFIG_MULTICAST_ADDRESS_FILITER)
    {
...
int FGmacLwipPortRxComplete(FGmacLwipPort *instance_p)
{   
    u32 temp=0;
    SYS_ARCH_DECL_PROTECT(lev);
    SYS_ARCH_PROTECT(lev);
    temp=instance_p->ri_is_call;
    instance_p->ri_is_call=0;
...
void *FGmacLwipPortRx(FGmacLwipPort *instance_p)
{
    ...
    u32 desc_buffer_last_index;/* For Last Desc buffer buf position */

    u32 rx_buffer_size = FGMAC_MAX_PACKET_SIZE & GENMASK(10, 0); /* Size of a rx buffer */
    FGmac *gmac_p;
    ...
    desc_buffer_last_index = gmac_p->rx_ring.desc_buf_idx;
    length = (gmac_p->rx_desc[desc_buffer_last_index].status & FGMAC_DMA_RDES0_FRAME_LEN_MASK) >> FGMAC_DMA_RDES0_FRAME_LEN_SHIFT;
    buffer = (u8 *)(intptr)(gmac_p->rx_desc[gmac_p->rx_ring.desc_first_buf_idx].buf_addr);
  ...
  length += ETH_PAD_SIZE; /* allow room for Ethernet padding */
    ...
    if (p != NULL)
    {
    ...
    #endif
        desc_buffer_index = gmac_p->rx_ring.desc_first_buf_idx;
        dma_rx_desc = &gmac_p->rx_desc[desc_buffer_index];


        bytes_left_to_copy = p->len;
        pay_load_offset = 0;
        /* Check if the length of bytes to copy in current pbuf is bigger than Rx buffer size*/
        while (bytes_left_to_copy > rx_buffer_size)
        {
            /* Copy data to pbuf */
            memcpy((u8 *)((u8 *)p->payload + pay_load_offset), (u8 *)buffer, rx_buffer_size);

            bytes_left_to_copy = bytes_left_to_copy - rx_buffer_size;
            pay_load_offset = pay_load_offset + rx_buffer_size;
        ...
        /* Copy remaining data to pbuf */
        memcpy((u8 *)((u8 *)p->payload + pay_load_offset),(u8 *)buffer, bytes_left_to_copy);
    ...
    for (desc_buffer_index = gmac_p->rx_ring.desc_first_buf_idx; desc_buffer_index != gmac_p->rx_ring.desc_idx; FGMAC_DMA_INC_DESC(desc_buffer_index, gmac_p->rx_ring.desc_max_num))
    {
        dma_rx_desc = &gmac_p->rx_desc[desc_buffer_index];
...
enum lwip_port_link_status FGmacPhyStatus(struct LwipPort *gmac_netif_p)
{
    ...
    if(gmac_p->phy_link_status == FGMAC_NEGOTIATION)
    {
        ...
        {
            FGMAC_LWIP_PORT_GMAC_DEBUG_E("FGmacPhyCfgInitialize is error \r\n");
            return ETH_LINK_DOWN;
        }
