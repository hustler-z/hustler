 ...
 *
 *
 * FilePath: serial_ddma_flow_example.c
 * Date: 2023-04-12 14:53:42
 * LastEditTime: 2023-06-21 17:57:24
 * Description:  This file is for serial transfer in flow mode under control of DDMA example 
...
static FError FSerialDdmaFlowInit()
{
    FError ret;
    ...
    FPl011SetOptions(&uart_instance, 
                     ...
                     FPL011_OPTION_CTSEN | FPL011_OPTION_TXDMAEN | 
...
static FError FSerialDdmaFlowDeinit()
{
    FError ret;
...
int FSerialDdmaFlowExample()
{
    int ret = 0;

    /* hardware init */
    ret = FSerialDdmaFlowInit();
    if (ret != FT_SUCCESS)
    {
    ...
    if (0 == ret)
    {
        printf("%s@%d: Serial ddma in flow mode example success !!! \r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: Serial ddma in flow mode example failed !!!, ret = %d \r\n", __func__, __LINE__, ret);
    }
