 ...
 *
 *
 * FilePath: i2s_dp_tx_example.c
 * Created Date: 2023-07-10 10:25:00
 * Last Modified: 2023-12-21 16:21:55
 * Description:  This file is for the I2S DP TX example function implmentation.
 *
 * Modify History:
...
/***************** functions*************************************************/
static void FI2sTransFlowCallback(void *param)
{
    FI2s *instance_p = (FI2s *)param;
    return;
}

static void FI2sTransIdleCallback(void *param)
{
    FI2s *instance_p = (FI2s *)param;
    return;
}
...
static void FI2sIrqSet(FI2s *instance_p)
{
    ...
    FI2sRegisterInterruptHandler(instance_p, FI2S_INTR_TRANS_FO, FI2sTransFlowCallback, (void *)instance_p);
    FI2sRegisterInterruptHandler(instance_p, FI2S_INTR_TRANS_FE, FI2sTransIdleCallback, (void *)instance_p);
...
static void FI2sIrqAllEnable(FI2s *instance_p)
{
    ...
    for (event_type = 0; event_type < FI2S_INTR_TRANS_FE; event_type++)
    {
...
FError FI2sTxEs8336Init(u32 work_mode, u32 word_length)
{
...
FError FI2sTxInit(u32 work_mode, u32 word_length)
{
    FError ret = FI2S_SUCCESS;
    FI2s *instance = &i2s_ctrl;
    u32 i2s_id = 0; /* 默认使用I2S-0 */
...
FError FI2sDdmaMemoryToDeviceCopy(u32 chan_id, u32 work_mode, uintptr src, fsize_t total_bytes, fsize_t per_buff_len)
{
	FError ret = FI2S_SUCCESS;
	...
	ddma_chan_config.dev_addr = i2s_config.base_addr + FI2S_TXDMA ;
	...
	FDdmaRegisterChanEvtHandler(&ddmac, chan_id, FDDMA_CHAN_EVT_REQ_DONE, FDdmaTxDMADone, NULL);
...
FError FI2sDdmaDpTxExample(void)
{
    FError ret = FI2S_SUCCESS;
    const u32 ddma_id = FDDMA4_DP1_I2S_ID; /* I2S所绑定的DDMA默认是DDMA-2 */
    const u32 channel = 0; /* 发送通道为DDMA通道0 */
    u32 total_byte = 4096 * 16384 ; /* 播放音频的最大长度，此值为自定义 */
    u32 per_buffer_len = 16384; /* 每一个BDL描述符所负责的数据长度，请保持total_byte是per_buffer_len的倍数 */
    u32 work_mode = AUDIO_PCM_STREAM_PLAYBACK; /* playback mode */
    u32 word_length = AUDIO_PCM_STREAM_WORD_LENGTH_16; /* 16-bits word length */

    ret = FI2sTxEs8336Init(work_mode, word_length);
    if (FES8336_SUCCESS != ret)
    {
    ...
    ret = FI2sTxInit(work_mode, word_length);
    if (FI2S_SUCCESS != ret)
    {
        printf("Init the I2S failed.\r\n");
        return ret;
    ...
    FI2sDdmaMemoryToDeviceCopy(channel, work_mode, (uintptr)rx_buf, total_byte, per_buffer_len);

    return ret;
}

FError FI2sDdmaDpTxStopWork(void)
{
    FError ret = FES8336_SUCCESS;
