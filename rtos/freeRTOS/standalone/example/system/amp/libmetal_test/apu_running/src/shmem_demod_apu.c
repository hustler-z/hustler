...
#include "fdebug.h"
#define LIBMETAL_SH_APU_DEBUG_TAG "LIBMETAL_SH_APU"
#define LIBMETAL_SH_APU_DEBUG_I(format, ...) FT_DEBUG_PRINT_I(LIBMETAL_SH_APU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_SH_APU_DEBUG_W(format, ...) FT_DEBUG_PRINT_W(LIBMETAL_SH_APU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_SH_APU_DEBUG_E(format, ...) FT_DEBUG_PRINT_E(LIBMETAL_SH_APU_DEBUG_TAG, format, ##__VA_ARGS__)
...
int shmem_echo(struct metal_io_region *shm_io)
{
    ...
    LIBMETAL_SH_APU_DEBUG_I("Setting up shared memory demo.\n");
    ...
    LIBMETAL_SH_APU_DEBUG_I("Starting shared memory demo.\n");
    ...
    if (!tx_data)
    {
        LIBMETAL_SH_APU_DEBUG_E("Failed to allocate memory.\n");
        ret = -1;
        goto out;
    }
    ...
    LIBMETAL_SH_APU_DEBUG_I("Sending message: %s\n",
    ...
    if (ret < 0)
    {
        LIBMETAL_SH_APU_DEBUG_E("Unable to metal_io_block_write().\n");
        goto out;
    }
    ...
    metal_io_block_read(shm_io, SHM_RX_BUFFER_OFFSET, rx_data, data_len);
    if (ret < 0)
    {
        LIBMETAL_SH_APU_DEBUG_E("Unable to metal_io_block_read().\n");
        goto out;
    }
    ...
    if (ret)
    {
        LIBMETAL_SH_APU_DEBUG_E("Received data verification failed.\n");
        LIBMETAL_SH_APU_DEBUG_I("Expected:");
        FtDumpHexByte(tx_data, data_len);
        LIBMETAL_SH_APU_DEBUG_I("Actual:");
        FtDumpHexByte(rx_data, data_len);
    }
    else
    {
        LIBMETAL_SH_APU_DEBUG_I("Message Received: %s\n", (char *)(rx_data + sizeof(*msg_hdr)));
    }
...
out:
    ...
    LIBMETAL_SH_APU_DEBUG_I("Shared memory demo: %s.\n", ret ? "Failed" : "Passed");
    return ret;
}
...
int shmem_demo()
{
    ...
    LIBMETAL_SH_APU_DEBUG_I("shared memory.");

    /* Open the shared memory device */
    ret = metal_device_open(BUS_NAME, SHM_DEV_NAME, &device);
    if (ret)
    {
        LIBMETAL_SH_APU_DEBUG_E("Failed to open device %s.\n", SHM_DEV_NAME);
        return ret;
    }
    ...
    if (!io)
    {
        LIBMETAL_SH_APU_DEBUG_E("Failed to get io region for %s.\n", device->name);
        ret = -ENODEV;
        goto out;
    }
