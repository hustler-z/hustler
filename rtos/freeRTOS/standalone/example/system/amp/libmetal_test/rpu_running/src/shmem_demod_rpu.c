 ...
 * 1.1  liushengming    2023/03/09  modified function of shmem_demod
 */
...
#include "fdebug.h"
#define LIBMETAL_SH_RPU_DEBUG_TAG "LIBMETAL_SH_RPU"
#define LIBMETAL_SH_RPU_DEBUG_I(format, ...) FT_DEBUG_PRINT_I(LIBMETAL_SH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_SH_RPU_DEBUG_W(format, ...) FT_DEBUG_PRINT_W(LIBMETAL_SH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_SH_RPU_DEBUG_E(format, ...) FT_DEBUG_PRINT_E(LIBMETAL_SH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)
...
static int shmem_echod(struct metal_io_region *shm_io)
{
    ...
    if (!data)
    {
        LIBMETAL_SH_RPU_DEBUG_E("Failed to allocate memory.\r\n");
        return -1;
    }

    LIBMETAL_SH_RPU_DEBUG_I("Wait for shared memory demo to start.\r\n");
    while (metal_io_read32(shm_io, SHM_DEMO_CNTRL_OFFSET) !=
           DEMO_STATUS_START)
        ;

    LIBMETAL_SH_RPU_DEBUG_I("Demo has started.\r\n");
    /* wait for message is available */
    while (metal_io_read32(shm_io, SHM_DEMO_CNTRL_OFFSET) ==
           DEMO_STATUS_START)
    {
        if (metal_io_read32(shm_io, SHM_RX_AVAIL_OFFSET) == rx_count)
        {
            continue;
        }
        /* Message is available, read the message header */
        ret = metal_io_block_read(shm_io, SHM_RX_BUFFER_OFFSET, data, sizeof(struct msg_hdr_s));
        if (ret < 0)
        {
            LIBMETAL_SH_RPU_DEBUG_E("Unable to metal_io_block_read().\n");
            return ret;
        }
        ...
        len = msg_hdr->len;
        if (msg_hdr->len > (SHM_BUFFER_SIZE - sizeof(*msg_hdr)))
        {
            LIBMETAL_SH_RPU_DEBUG_E("Input message is too long %u.\n",msg_hdr->len);
            len = SHM_BUFFER_SIZE - sizeof(*msg_hdr);
        }
        /* Read the message data */
        ret = metal_io_block_read(shm_io, SHM_RX_BUFFER_OFFSET + sizeof(*msg_hdr), data + sizeof(*msg_hdr), len);

        rx_count++;
        ret = metal_io_block_write(shm_io, SHM_TX_BUFFER_OFFSET, (void *)data, sizeof(*msg_hdr) + len);
        if (ret < 0)
        {
            LIBMETAL_SH_RPU_DEBUG_E("Unable to metal_io_block_write().\n");
            return ret;
        }
    ...
    LIBMETAL_SH_RPU_DEBUG_I("Shared memory test finished.\r\n");
    return 0;
}
...
int shmem_demod(void)
{
    ...
    LIBMETAL_SH_RPU_DEBUG_I("Shared memory.");

    /* Open shared memory device */
    ret = metal_device_open(BUS_NAME, SHM_DEV_NAME, &device);
    if (ret)
    {
        LIBMETAL_SH_RPU_DEBUG_E("Failed to open device %s.\n", SHM_DEV_NAME);
        goto out;
    }
    ...
    if (!io)
    {
        LIBMETAL_SH_RPU_DEBUG_E("Failed to map io region for %s.\n", device->name);
        ret = -ENODEV;
        goto out;
    }
