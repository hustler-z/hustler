...
int rpmsg_echo(struct rpmsg_device *rdev, void *priv)
{
    ...
    metal_assert(priv);
    int ret;
    ECHO_DEV_SLAVE_DEBUG_I("Starting rpmsg_echo application...\r\n");
    ret = app(rdev, priv);
    if (ret != 0)
    {
        ECHO_DEV_SLAVE_DEBUG_E("Rpmsg_echo application error,code:0x%x",ret);
        return ret;
    }

    ECHO_DEV_SLAVE_DEBUG_I("Stopping rpmsg_echo application...\r\n");
