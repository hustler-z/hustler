...
#define RADAX_SHMEM_PROTO_UNDEFINED	0x0000
...
#define COMM_REGION_GENERIC_HEADER					\
    ...
    /** Holds static flags, see RADAX_COMM_FLAG_*. */		\
    U32 flags;							\
    /** Debug console that may be accessed by the inmate. */	\
    struct RadaxConsole console;				\
    /** Base address of PCI memory mapped config. */		\
    U64 pciMmconfigBase;

struct RadaxCommRegion {
...
#define commRegion	((struct RadaxCommRegion *)COMM_REGION_BASE)

enum MapType { MAP_CACHED, MAP_UNCACHED };

#define RADAX_INMATE_MEM_PAGE_DIR_LEN	512  // diff
#define PAGE_SIZE	                    (4 * 1024ULL)

static U64 __attribute__((aligned(4096)))
    pageDirectory[RADAX_INMATE_MEM_PAGE_DIR_LEN];
