                                         ...
                                         struct wlan_network *network)
{
    ...
    if (is_state(CM_STA_CONNECTED))
    {
        ...
        wlan.sta_ipv6_state = CM_STA_IDLE;
#endif
        if ((t_ptr)msg->data == IEEEtypes_REASON_MIC_FAILURE)
        {
...
static enum cm_uap_state uap_state_machine(struct wifi_message *msg)
{
    ...
    switch (msg->event)
    {
        case CM_UAP_USER_REQUEST_START:
            wlan.cur_uap_network_idx = (t_ptr)msg->data;
            wlan.scan_count          = 0;
...
{
    int ret;
    struct wlan_network *new_network = &wlan.networks[(t_ptr)msg->data];
#ifdef CONFIG_WPA_SUPP
    struct netif *netif = net_get_sta_interface();
#endif
...
#ifndef CONFIG_WPA_SUPP
    ret = do_connect((int)(t_ptr)msg->data);
#else
    wlan.scan_count      = 0;
...
static void wlcm_request_reconnect(enum cm_sta_state *next, struct wlan_network *network)
{
    ...
    msg.data   = (void *)(t_ptr)(wlan.cur_network_idx);
...
{
    uint32_t wakeup_condition = (uint32_t)(t_ptr)msg->data;
...
static enum cm_sta_state handle_message(struct wifi_message *msg)
{
    ...
    switch (msg->event)
    {
        ...
        case CM_STA_USER_REQUEST_PS_ENTER:
            ...
            wlan_enable_power_save((int)(t_ptr)msg->data);
            break;
        case CM_STA_USER_REQUEST_PS_EXIT:
            ...
            wlan_disable_power_save((int)(t_ptr)msg->data);
            break;
...
static void cm_main(os_thread_arg_t data)
{
    ...
    while (true)
    {
        ...
        if (ret == WM_SUCCESS)
        {
            ...
            {
                wlcm_deinit((int)(t_ptr)msg.data);
            }
            else
            {
                /* STA related msg */
...
int wlan_scan_with_opt(wlan_scan_params_v2_t t_wlan_scan_param)
{
    ...
    ret = send_user_request(CM_STA_USER_REQUEST_SCAN, (unsigned int)(t_ptr)wlan_scan_param);
...
int _wlan_rrm_scan_cb(unsigned int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ((i == count - 1U) && (buf_pos > rep_buf))
        {
            ...
            {
                *(char *)(t_ptr)pos_last_indication = (char)1U;
            }
            meas_report_len = buf_pos - rep_buf;
