...
int wifi_set_packet_filters(wifi_flt_cfg_t *flt_cfg)
{
    ...
    for (i = 0; i < flt_cfg->nentries; i++)
    {
        ...
        if (filter_buf != NULL)
            entry_hdr->len = (t_ptr)filter_buf - (t_ptr)entry_hdr - sizeof(mef_entry_header);
    }
...
int wifi_get_cal_data(wifi_cal_data_t *cal_data)
{
    ...
    HostCmd_DS_802_11_CFG_DATA *cfg_data_cmd = (HostCmd_DS_802_11_CFG_DATA *)((t_ptr)cmd + S_DS_GEN);
                            ...
                            uint8_t *custom_ie)
{
    ...
    HostCmd_DS_SYS_CONFIG *sys_config_cmd = (HostCmd_DS_SYS_CONFIG *)((t_ptr)cmd + S_DS_GEN);
