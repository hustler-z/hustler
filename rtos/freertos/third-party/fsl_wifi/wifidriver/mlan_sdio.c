...
int sdio_drv_creg_read(int addr, int fn, uint32_t *resp)
{
    ...
    if (SDIO_IO_Read_Direct(&wm_g_sd.card, (sdio_func_num_t)fn, (uint32_t)addr, (uint8_t *)resp) != KOSA_StatusSuccess)
    {
...
bool sdio_drv_creg_write(int addr, int fn, uint8_t data, uint32_t *resp)
{
    ...
    if (SDIO_IO_Write_Direct(&wm_g_sd.card, (sdio_func_num_t)fn, (uint32_t)addr, &data, true) != KOSA_StatusSuccess)
    {
        (void)OSA_MutexUnlock(&sdio_mutex);
        return false;
    }
...
int sdio_drv_read(uint32_t addr, uint32_t fn, uint32_t bcnt, uint32_t bsize, uint8_t *buf, uint32_t *resp)
{
    ...
    if (SDIO_IO_Read_Extended(&wm_g_sd.card, (sdio_func_num_t)fn, addr, buf, param, flags) != KOSA_StatusSuccess)
    {
        (void)OSA_MutexUnlock(&sdio_mutex);
        return 0;
    }
...
bool sdio_drv_write(uint32_t addr, uint32_t fn, uint32_t bcnt, uint32_t bsize, uint8_t *buf, uint32_t *resp)
{
    ...
    if (SDIO_IO_Write_Extended(&wm_g_sd.card, (sdio_func_num_t)fn, addr, buf, param, flags) != KOSA_StatusSuccess)
    {
        (void)OSA_MutexUnlock(&sdio_mutex);
        return false;
    }
...
{
    (void)memset(&wm_g_sd, 0, sizeof(sdmmc_sdio_t));

    BOARD_WIFI_BT_Config(&wm_g_sd, SDIO_CardInterruptCallBack);

#if defined(SD_TIMING_MAX)
    wm_g_sd.card.currentTiming = SD_TIMING_MAX;
#endif
#if defined(SD_CLOCK_MAX)
    wm_g_sd.card.usrParam.maxFreq = SD_CLOCK_MAX;
#endif
}
...
static int sdio_card_init(void)
{
    ...
    if (SDIO_HostInit(&wm_g_sd.card) != KOSA_StatusSuccess)
    {
    ...
    wm_g_sd.card.usrParam.ioVoltage = NULL;
#elif defined(SDMMCHOST_OPERATION_VOLTAGE_1V8)
    /* Switch to 1.8V */
    if ((wm_g_sd.card.usrParam.ioVoltage != NULL) && (wm_g_sd.card.usrParam.ioVoltage->type == kSD_IOVoltageCtrlByGpio))
    {
        if (wm_g_sd.card.usrParam.ioVoltage->func != NULL)
        {
            wm_g_sd.card.usrParam.ioVoltage->func(kSDMMC_OperationVoltage180V);
        }
    }
#if SDMMCHOST_SUPPORT_VOLTAGE_CONTROL
    else if ((wm_g_sd.card.usrParam.ioVoltage != NULL) && (wm_g_sd.card.usrParam.ioVoltage->type == kSD_IOVoltageCtrlByHost))
    {
    ...
    wm_g_sd.card.operationVoltage = kSDMMC_OperationVoltage180V;
#endif

    BOARD_WIFI_BT_Enable(true);

    ret = SDIO_CardInit(&wm_g_sd.card);
    if (ret != WM_SUCCESS)
    {
        return ret;
    }
    ...
    (void)SDIO_SetBlockSize(&wm_g_sd.card, (sdio_func_num_t)0, 256);
    (void)SDIO_SetBlockSize(&wm_g_sd.card, (sdio_func_num_t)1, 256);
    (void)SDIO_SetBlockSize(&wm_g_sd.card, (sdio_func_num_t)2, 256);
