 ...
 * Last Modified: 2023-12-21 16:20:17
 * Description:  This file is for the I2S board example cmd catalogue.
 *
 * Modify History:
...
static int FI2SCmdEntry(int argc, char *argv[])
{
    ...
    {
        ret = FI2sDdmaRxExample();
        if (ret != FT_SUCCESS)
        {
            printf("I2S transmmit error: 0x%x\r\n", ret);
            return ret;
        }
    }
    ...
    {
        ret = FI2sDdmaRxStopWork();
        ret = FI2sDdmaTxStopWork();

        if (ret != FT_SUCCESS)
        {
            printf("I2S stop error: 0x%x\r\n", ret);
            return ret;
        }
    }
...
SHELL_EXPORT_CMD(SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN), i2s, FI2SCmdEntry, I2S board example);
#endif