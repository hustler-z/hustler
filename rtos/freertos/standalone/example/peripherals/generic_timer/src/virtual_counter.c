 ...
 *
 *
 * FilePath: virtual_counter.c
 * Date: 2023-05-56 08:37:22
 * LastEditTime: 2023-05-27 11:00:53
 * Description:  This file is for the virtual timer counter test example functions.
 * Modify History:
...
static void FGenericVirtualTimerHandler(s32 vector, void *param)
{
	...
	delta = GenericTimerRead(GENERIC_TIMER_ID1);

	expected_ticks = delta + ticks_per_beat;

#if SET_TIMER_VALUE
	GenericTimerSetTimerValue(GENERIC_TIMER_ID1, ticks_per_beat);
#else	
	GenericTimerSetTimerCompareValue(GENERIC_TIMER_ID1, expected_ticks);
#endif
	if(count == 1)
		goto next;
	...
	f_printk("Virtual Timer count: %ld, interval ticks: %lld\r\n",
			count,
			delta - delta_old);
next:
...
void FGenericVirtualTimerStart(void)
{
    ...
    InterruptSetPriority(GENERIC_VTIMER_IRQ_NUM, 0);
	...
	expected_ticks = GenericTimerRead(GENERIC_TIMER_ID1) + ticks_per_beat;
