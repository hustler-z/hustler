 ...
 *
 *
 * FilePath: timer_once_timing_example.c
 * Date: 2022-02-10 14:53:42
 * LastEditTime: 2022-02-17 17:46:03
 * Description:  This file is for timer once timing example function implmentation
 *
 * Modify History:
...
int FTimerOnceTimingExample()
{
    FError ret = 0;
    u32 id = TIMER_EXAMPLE_TEST_ID;

    printf("Once timer id:%d start... \r\n", id);
    /*1 second once timing*/
    ret = FTimerFunctionInit(id, 0, 2000000); 
    ...
    if (FTIMER_TACHO_SUCCESS == ret)
    {
        printf("%s@%d: timer once timing example test [success].\r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: timer once timing example test [failure].\r\n", __func__, __LINE__);
    }
