 ...
 *
 *
 * FilePath: sata_controller_pio_intr_example.c
 * Date: 2022-02-10 14:53:42
 * LastEditTime: 2022-02-17 17:46:03
 * Description:  This file is for sata controller pio intr example functions
 *
 * Modify History:
...
/************************** Function *****************************************/
/* function of sata controller pio intr example */

int FSataControllerPioIntrExample()
{
    ...
    /*init sata controller intr*/ 
    FSataControllerIrqInit(ahci_host);
    
    printf("Test sata read and write in host %d block %d\n", ahci_host, start_blk);

    /*write data*/
    char write_buf[FSATA_BLOCK_SIZE] = "Sata Controller Pio Intr Test Example Write Successfully!!!";
    ...
    /*deinit sata controller interrupt*/
    FSataControllerIrqDeInit(ahci_host);
    ...
    if (ret == FT_SUCCESS)
    {
        printf("%s@%d: Sata controller pio intr example success !!! \r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: Sata controller pio intr example failed !!!, ret = %d \r\n", __func__, __LINE__, ret);
        return FSATA_ERR_OPERATION;
    }
