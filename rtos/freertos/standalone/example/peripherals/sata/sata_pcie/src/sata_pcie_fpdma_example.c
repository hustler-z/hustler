 ...
 *
 *
 * FilePath: sata_pcie_fpdma_example.c
 * Date: 2022-02-10 14:53:42
 * LastEditTime: 2022-02-17 17:46:03
 * Description:  This file is for sata pcie fpdma example function implmentation
 *
 * Modify History:
...
/* function of sata pcie fpdma example */
int FSataPcieFpdmaExample()
{
    ...
    /*write data*/
    u8 write_buf[FSATA_BLOCK_SIZE] = "Sata Pcie Fpdma Write Successfully!!!";
    ret = FSataPcieReadWrite(ahci_host, start_blk, blk_num, write_buf, SATA_FPDMA_WRITE_MODE);
    if (ret != FT_SUCCESS)
    {
        FSATA_ERROR("Sata pcie fpdma write failed !!! \r\n");
        return FSATA_ERR_OPERATION;
    }
    
    /*read data*/
    char read_buf[FSATA_BLOCK_SIZE];
    ret = FSataPcieReadWrite(ahci_host, start_blk, blk_num, read_buf, SATA_FPDMA_READ_MODE);
    if (ret != FT_SUCCESS)
    {
        FSATA_ERROR("Sata pcie fpdma read failed !!! \r\n");
        return FSATA_ERR_OPERATION;
    }
    /*printf the read_buf*/
    FtDumpHexByte(read_buf, FSATA_BLOCK_SIZE * blk_num);
    /*deinit sata and pcie*/
    FSataPcieDeinit(ahci_host);
    /* print message on example run result */
    if (ret == FT_SUCCESS)
    {
        printf("%s@%d: Sata pcie fpdma example success !!! \r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: Sata pcie fpdma example failed !!!, ret = %d \r\n", __func__, __LINE__, ret);
        return FSATA_ERR_OPERATION;
    }
