 ...
 * 
 * 
 * FilePath: lwip_jumbo_example.c
 * Created Date: 2023-11-08 16:05:47
 * Last Modified: 2023-12-08 14:59:37
 * Description:  This file is for lwip jumbo example function implementation.
 * 
 * Modify History:
...
#define CONFIG_DEFAULT_INIT(config,driver_config,instance_id,interface_type)           \
		...
		.config.capability = LWIP_PORT_MODE_JUMBO, 
...
int LwipJumboCreate(void)
{
    ...
    if (ret == FT_SUCCESS)
    {
        printf("%s@%d: Lwip jumbo example success !!! \r\n", __func__, __LINE__);
        printf("[system_example_pass]\r\n");
    }
    else
    {
        printf("%s@%d: Lwip jumbo example failed !!!, ret = %d \r\n", __func__, __LINE__, ret);
    }
    return 0;
}

void LwipJumboDeinit(void)
{
    for (int i = 0; i < MAC_NUM; i++)
    {
        struct netif *netif_p = NULL;
