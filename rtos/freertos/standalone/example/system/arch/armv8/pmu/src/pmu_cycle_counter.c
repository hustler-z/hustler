 ...
 * 
 * 
 * FilePath: pmu_cycle_counter.c
 * Created Date: 2023-11-06 15:49:49
 * Last Modified: 2024-03-01 10:57:38
 * Description:  This file is for cycle counter test
 * 
 * Modify History:
...
static int IsEventSupported(u64 bitmap, u32 event_id) 
{
...
void FPmuCycleCntExample(void) 
{
    FError error;
    u64 value_after;
    u32 timeout_cnt = 0;
    u32 event_id = 0; /* Replace with your specific event ID */
    fsize_t counter_id = FPMU_CYCLE_COUNT_IDX; /* Replace with your specific counter ID */
    irq_flg = 0 ;
    /* Initialize the PMU */
    ...
    if (IsEventSupported(instance.event_id_bitmap, event_id) ||
        IsEventSupported(instance.event_ext_id_bitmap, event_id)) 
    {
        ...
        error = FPmuWriteCounter(&instance, counter_id, FPMU_PERIOD_CALC_64BIT(1000));
        if (error != FPMU_SUCCESS) {
        ...
        error = FPmuReadCounter(&instance, counter_id, &value_after);
        if (error != FPMU_SUCCESS) {
