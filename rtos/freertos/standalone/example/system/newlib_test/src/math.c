...
double thedouble(long msw, long lsw);
       ...
       int merror)
{
  ...
  unsigned long sw;
  memcpy(&sw, &(isbits.value), sizeof(unsigned long));
  isbits.parts.msw = (sw >> 32);
  isbits.parts.lsw = (sw & 0xFFFFFFFF);
#endif
  correct.parts.msw = p->qs[0].msw;
  correct.parts.lsw = p->qs[0].lsw;

#if defined(__aarch64__) 
  correct.value = thedouble(correct.parts.msw, correct.parts.lsw);
#endif  
...
#if defined(__aarch64__) 
  double convert = 0.0;
  unsigned long sw = ((msw << 32) | lsw);
  memcpy(&convert, &sw, sizeof(unsigned long));
  return convert;
#endif
  return x.value;
}
