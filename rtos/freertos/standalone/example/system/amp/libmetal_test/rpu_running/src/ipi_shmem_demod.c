...
#include "fdebug.h"
#define LIBMETAL_IPSH_RPU_DEBUG_TAG "LIBMETAL_IPSH_RPU"
#define LIBMETAL_IPSH_RPU_DEBUG_I(format, ...) FT_DEBUG_PRINT_I(LIBMETAL_IPSH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_IPSH_RPU_DEBUG_W(format, ...) FT_DEBUG_PRINT_W(LIBMETAL_IPSH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)
#define LIBMETAL_IPSH_RPU_DEBUG_E(format, ...) FT_DEBUG_PRINT_E(LIBMETAL_IPSH_RPU_DEBUG_TAG, format, ##__VA_ARGS__)

#define BUF_SIZE_MAX 512
#define SHUTDOWN "shutdown"

/* Shared memory offsets */
/* sys_init.c ==> shm -> metal_device.regions.size >= SHM_BUFF_OFFSET_TX + BUF_SIZE_MAX */
						   ...
						   struct metal_io_region *shm_io)
{
	...
	if (!lbuf)
	{
		LIBMETAL_IPSH_RPU_DEBUG_E("Failed to allocate local buffer for msg.\n");
		ret = -ENOMEM;
		goto out;
	}
	/* Clear shared memory */
	...
	LIBMETAL_IPSH_RPU_DEBUG_I("Wait for echo test to start tx_data_offset 0x%x,rx_data_offset 0x%x.\n",tx_data_offset,rx_data_offset);
	rx_count = 0;
	while (1)
	{
		wait_for_notified(&remote_nkicked);
		rx_avail = metal_io_read32(shm_io, rx_avail_offset);
		LIBMETAL_IPSH_RPU_DEBUG_I("rx_avail_offset:0x%x, rx_avail:%d.\r\n",rx_avail_offset,rx_avail);
		while (rx_count != rx_avail)
		{
			 ...
			 * rx address array.
			 */
			buf_phy_addr_32 = metal_io_read32(shm_io,rx_addr_offset);
			rx_data_offset = metal_io_phys_to_offset(shm_io, (metal_phys_addr_t)buf_phy_addr_32);
			if (rx_data_offset == METAL_BAD_OFFSET)
			{
				LIBMETAL_IPSH_RPU_DEBUG_E("[%u]failed to get rx offset: 0x%x, 0x%lx.\n",rx_count, buf_phy_addr_32,
			...
			metal_io_block_read(shm_io, rx_data_offset, lbuf,sizeof(struct msg_hdr_s));
			msg_hdr = (struct msg_hdr_s *)lbuf;

			/* Check if the message header is valid */
			if (msg_hdr->len > (BUF_SIZE_MAX - sizeof(*msg_hdr)))
			{
				LIBMETAL_IPSH_RPU_DEBUG_E("wrong msg: length invalid: %u, %u.\n",
			...
			/* Read message */
			metal_io_block_read(shm_io,rx_data_offset,lbuf + sizeof(*msg_hdr), msg_hdr->len);
			...
			if (msg_hdr->len == strlen(SHUTDOWN) && !strncmp(SHUTDOWN, (lbuf + sizeof(struct msg_hdr_s)), strlen(SHUTDOWN)))
			{
				LIBMETAL_IPSH_RPU_DEBUG_I("Received shutdown message\n");
				goto out;
			}
			 ...
			 * the buffer address.
			 */
			buf_phy_addr_32 = (u32)metal_io_phys(shm_io, tx_data_offset);
...
out:
	LIBMETAL_IPSH_RPU_DEBUG_I("IPI with shared memory demo finished with exit code: %i.\n", ret);
...
int ipi_shmem_demod()
{
    ...
    if (ret)
    {
        LIBMETAL_IPSH_RPU_DEBUG_E("Failed to open device %s.\n", SHM_DEV_NAME);
        goto out;
    }
    ...
    if (!shm_io)
    {
        LIBMETAL_IPSH_RPU_DEBUG_E("Failed to map io region for %s.\n", shm_dev->name);
        ret = -ENODEV;
        goto out;
    }
