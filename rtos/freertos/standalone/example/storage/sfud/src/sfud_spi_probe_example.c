 ...
 *
 *
 * FilePath: sfud_spi_probe_example.c
 * Date: 2023-07-31 11:23:42
 * LastEditTime: 2023-08-1 18:47:54
 * Description:  This file is for sfud spi probe example function implementation.
 *
 * Modify History:
...
static int SfudInfo(void)
{
    ...
    {
        FSFUD_WARRN("Flash on spi-%d is not found!", SFUD_CONTROLLER_ID);
        return 1;
    }

    printf("Flash info on spi-%d:\r\n", SFUD_CONTROLLER_ID);
...
int FSfudSpiProbeExample(void)
{
    ...
    if (ret == 0)
    {
        printf("%s@%d: SFUD spi probe example [success].\r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: SFUD spi probe example [failure].\r\n", __func__, __LINE__);
    }
