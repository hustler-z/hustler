 ...
 *
 *
 * FilePath: sfud_qspi_wr_example.c
 * Date: 2023-07-31 11:23:42
 * LastEditTime: 2023-10-08 18:47:54
 * Description:  This file is for sfud qspi read write example function implementation.
 *
 * Modify History:
...
#define FLASH_WR_OFFSET     0x10000 /* Flash write address offset, protect firmware content */
...
int FSfudQspiWRExample(void)
{
    ...
    {
        FSFUD_ERROR("Flash on qspi controller %d is not found!", SFUD_CONTROLLER_ID);
    ...
    if (ret == 0)
    {
        printf("%s@%d: Sfud qspi read write example [success].\r\n", __func__, __LINE__);
        printf("[system_example_pass]\r\n");
    }
    else
    {
        printf("%s@%d: Sfud qspi read write example [failure].\r\n", __func__, __LINE__);
    }
