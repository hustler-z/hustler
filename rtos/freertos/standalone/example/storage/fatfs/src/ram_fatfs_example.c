 ...
 *
 *
 * FilePath: ram_fatfs_example.c
 * Date: 2023-08-07 14:53:42
 * LastEditTime: 2023-08-07 17:46:03
 * Description:  This file is for ram fatfs example function implmentation
 *
 * Modify History:
...
int FRamFatfsExample()
{
    ...
    u32 cyc = 3;
    /*set the mount point*/
    const char *mount_point = FF_RAM_DISK_MOUNT_POINT;
    /*set the mount point*/
    ...
    ret = ff_setup(&ram_fatfs, path_buff, &opt, TRUE);
    if (ret != 0)
    {
    ...
    /* print message on ram fatfs example run result */
    if (0 == ret)
    {
        printf("%s@%d: ram fatfs example [success].\r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: ram fatfs example [failure].\r\n", __func__, __LINE__);
    }
