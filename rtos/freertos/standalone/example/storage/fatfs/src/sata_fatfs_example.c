 ...
 *
 *
 * FilePath: sata_fatfs_example.c
 * Date: 2023-08-07 14:53:42
 * LastEditTime: 2023-08-07 17:46:03
 * Description:  This file is for sata fatfs example function implmentation
 *
 * Modify History:
...
/* function of sata fatfs example*/
int FSataFatfsExample()
{
    int ret = 0;
    ...
    u32 cyc = 3;
    /*set the mount point*/
    const char *mount_point = FF_SATA_DISK_MOUNT_POINT;
    /*set the mount point*/
    ...
    ret = ff_setup(&sata_fatfs, path_buff, &opt, TRUE);
    if (ret != 0)
    {
    ...
    /* print message on sata fatfs example run result */
    if (0 == ret)
    {
        printf("%s@%d: sata fatfs example [success].\r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: sata fatfs example [failure].\r\n", __func__, __LINE__);
    }
