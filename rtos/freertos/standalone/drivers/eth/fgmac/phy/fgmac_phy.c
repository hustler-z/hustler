...
static FError FGmacPhyAutoNegotiation(FGmac *instance_p, u32 phy_address)
{
    ...
    {
        reg_val = 0;
        ret = FGmacReadPhyReg(instance_p, phy_address, FGMAC_PHY_MII_PHY_SPECIAL_REG, &reg_val);
        if (FGMAC_SUCCESS != ret)
        {
            break;
        }
    ...
    while (!(reg_val & FGMAC_PHY_SPECIFIC_STATUS_LINK_MASK) && (0 <= --timeout));
...
static FError FGmacPhyNoneNegotiation(FGmac *instance_p, u32 phy_address)
{
    ...
    /* 设置速度bit6|bit13,01b-100M, 00b-10M */
    switch (instance_p->config.speed)
    {
        ...
        case FGMAC_PHY_SPEED_10:
            ...
            break;
        default:
            FGMAC_ERROR("The speed must be 100M or 10M after disabling auto negotiation.");
            FASSERT(0);
            break;
    }
...
FError FGmacPhyReset(FGmac *instance_p, u32 phy_address)
{
    ...
    if (FGMAC_SUCCESS != ret)
    {
        FGMAC_ERROR("Reset phy failed when writing.");
        return ret;
    }  
    ...
    {
        FDriverMdelay(500);
        ret  = FGmacReadPhyReg(instance_p, phy_address, FGMAC_PHY_MII_CTRL_REG, &control);
        if (FGMAC_SUCCESS != ret)
        {
            FGMAC_ERROR("Reset phy failed when reading.");
            return ret;
        }
        if((control & FGMAC_PHY_MII_CR_RESET) == 0)
            break;
    }
 ...
 * @param {FGmac} *instance_p, instance of FGmac controller
 * @param {u32} reset_flag, a flag which indicates whether to perform gmac phy reset
 * @return err code information, FGMAC_SUCCESS indicates success，others indicates failed
 */
FError FGmacPhyCfgInitialize(FGmac *instance_p,u32 reset_flag)
{
    FASSERT(instance_p);
    uintptr base_addr = instance_p->config.base_addr;
    FError ret = FGMAC_SUCCESS;
