                            ...
                            u8 *rx_buf, const fsize_t rx_pre_buf_len, const fsize_t rx_buf_num)
{
    ...
    rx_ring_p->desc_first_buf_idx=0; /* idx of first rx buf */
...
FError FGmacRecvFrame(FGmac *instance_p)
{
    ...
    u32 desc_cnt = 0;
    u32 first_flag = 0; /* to monitor whether the value of rx_ring->desc_first_buf_idx is valid or not*/
           ...
           (desc_cnt < rx_ring->desc_max_num))
    {
        ...
        /* cur_rx_desc is only the first descriptor to receive data */
        if (FGMAC_DMA_RDES0_FIRST_DESCRIPTOR == (FGMAC_DMA_RDES0_LAST_OR_FIRST_FLAG & cur_rx_desc->status))
        {
            rx_ring->desc_first_buf_idx = rx_ring->desc_idx;
            first_flag = 1;  
        }
         /* cur_rx_desc is the last descriptor to receive data */
        else if(FGMAC_DMA_RDES0_LAST_DESCRIPTOR == (FGMAC_DMA_RDES0_LAST_DESCRIPTOR & cur_rx_desc->status))
        {
            rx_ring->desc_buf_idx = rx_ring->desc_idx;
            /* cur_rx_desc is the first and last descriptor to receive data */
            if(FGMAC_DMA_RDES0_LAST_OR_FIRST_FLAG == (FGMAC_DMA_RDES0_LAST_OR_FIRST_FLAG & cur_rx_desc->status))
            {
                 rx_ring->desc_first_buf_idx = rx_ring->desc_idx;
                 first_flag = 1;
            }
            /* cur_rx_desc is only the last descriptor to receive data */
            else
            {
                /* No need to do anything */        
            }

            FASSERT(first_flag==1);
