...
#ifdef CONFIG_SKIP_SHBUF_IO_WRITE
	memcpy(hdr, &rp_hdr, sizeof(struct rpmsg_hdr));
#else
	/* Copy data to rpmsg buffer. */
	io = rvdev->shbuf_io;
	status = metal_io_block_write(io, metal_io_virt_to_offset(io, hdr), &rp_hdr, sizeof(rp_hdr));
...
#ifdef VIRTIO_CACHED_BUFFERS
			metal_cache_invalidate(RPMSG_LOCATE_DATA(rp_hdr),rp_hdr->len);
#endif
