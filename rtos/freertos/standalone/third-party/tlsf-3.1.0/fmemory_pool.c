 ...
 * Description:  This file is for memory pool API implmentation
 * 
 * Modify History: 
...
FError FMempInit(FMemp *memp, void *begin_addr, void *end_addr)
{
    ...
    {
        FMEMP_WARN("Memory tlsf_ptr already inited, you may lose access to original memory tlsf_ptr.");
    }
    ...
    {
        FMEMP_WARN("TLSF buf aleady exists, skip from allocate !!!");
    }
 ...
 * @note 需要初始化后才能调用，申请的空间不再使用后需要调用FMempFree释放
 */
