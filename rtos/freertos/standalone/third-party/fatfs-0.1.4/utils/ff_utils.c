...
FRESULT ff_setup(ff_fatfs *fatfs, const TCHAR *mount_point , const MKFS_PARM *opt, UINT force_format)
{
    ...
    else if (!strcmp(mount_point, FF_FSDIF_TF_DISK_MOUNT_POINT))
    {
#ifdef CONFIG_FATFS_SDMMC_FSDIF_TF
        /* register tf disk */
        ff_diskio_register_fsdif_tf(pvol);
    ...
    else if (!strcmp(mount_point, FF_FSDIF_EMMC_DISK_MOUNT_POINT))
    {
#ifdef CONFIG_FATFS_SDMMC_FSDIF_EMMC
        /* register emmc disk */
        ff_diskio_register_fsdif_emmc(pvol);
