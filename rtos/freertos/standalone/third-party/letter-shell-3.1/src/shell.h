    ...
    const ShellCommand                                 \
        shellCommand##_name SECTION("shellCommand") =  \
            {                                          \
                ...
                .data.cmd.exit_info = NULL}


/* exit code used in application */
typedef struct
{
    int exit_code;
    const char *exit_msg;
} ShellExitCode; /* shell 命令返回项 */

typedef struct
{
#define SHELL_EXIT_CODE_MAGIC_NUM      0xBEEFFEED
    unsigned int magic_num;
    unsigned int exit_code_num;
    const ShellExitCode *exit_codes;
} ShellExitInfo; /* shell 命令返回项集合 */

/**
 * @brief shell 命令返回值信息定义
 * 
 * @param _name 命令名
 */
#define SHELL_EXPORT_EXIT_MSG(_name) \
    const ShellExitCode shellExitCodes##_name[]

/**
 * @brief shell 命令定义 (带返回值信息)
 * 
 * @param _attr 命令属性
 * @param _name 命令名
 * @param _func 命令函数
 * @param _desc 命令描述
 */
#define SHELL_EXPORT_CMD_MSG(_attr, _name, _func, _desc)                                   \
    const char shellCmd##_name[] = #_name;                                                 \
    const char shellDesc##_name[] = #_desc;                                                \
    const ShellExitInfo shellExit##_name = {                                               \
        .magic_num = SHELL_EXIT_CODE_MAGIC_NUM,                                            \
        .exit_code_num = sizeof(shellExitCodes##_name) / sizeof(shellExitCodes##_name[0]), \
        .exit_codes = shellExitCodes##_name};                                              \
    const ShellCommand                                                                     \
        shellCommand##_name SECTION("shellCommand") =                                      \
            {                                                                              \
                .attr.value = _attr,                                                       \
                ...
                .data.cmd.function = (int (*)())_func,                                     \
                .data.cmd.desc = shellDesc##_name,                                         \
                .data.cmd.exit_info = &shellExit##_name}
...
typedef struct shell_def
{
    ...
    {
        void *base;                    /**< 命令表基址 */
        unsigned /*short*/ long count; /**< 命令数量 */
    } commandList;
    struct
    {
...
typedef struct shell_command
{
    ...
    union
    {
        struct
        {
            ...
            const ShellExitInfo *exit_info; /**< 返回值信息 */
        } cmd;                /**< 命令定义 */
        struct
        {
            const char *name; /**< 变量名 */
