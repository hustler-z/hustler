...
#define SHELL_ARCH          "Armv8-aarch32   "
#elif defined(CONFIG_TARGET_ARMV8_AARCH64)
#define SHELL_ARCH          "Armv8-aarch64   "
#else
#define SHELL_ARCH          "Unkonwn  "
#endif

#if defined(CONFIG_USE_BAREMETAL_AMP)

#if defined(CONFIG_TARGET_AMP_MASTER)
#define SHELL_AMP_ROLE      "Amp-Master"
#elif defined(CONFIG_TARGET_AMP_SLAVE)
#define SHELL_AMP_ROLE      "Amp-Slave"
#else
#define SHELL_AMP_ROLE      "Amp-Role-Unkonwn"
#endif
...
static void shellWriteExitInfo(Shell *shell, ShellCommand *command, int value);
...
void shellInit(Shell *shell, char *buffer, unsigned short size)
{
    ...
    for (short i = 0; i < SHELL_HISTORY_MAX_NUMBER; i++)
    {
        ...
        shell->commandList.count = ((unsigned long)(&_shell_command_end)
                                ...
                                / (unsigned long)sizeof(ShellCommand);
    #else
...
void shellWriteVersion(Shell *shell)
{
...
void shellListCommand(Shell *shell)
{
    ...
    for (unsigned short i = 0; i < shell->commandList.count; i++)
    {
...
void shellListVar(Shell *shell)
{
    ...
    for (unsigned long i = 0; i < shell->commandList.count; i++)
    {
        if (base[i].attr.attrs.type > SHELL_TYPE_CMD_FUNC
    ...
    else
    {
        ...
        case SHELL_TYPE_VAR_CHAR:
            *((char *)(command->data.var.value)) = value;
            break;
        case SHELL_TYPE_VAR_STRING:
            shellStringCopy(((char *)(command->data.var.value)), (char *)(unsigned long)value);
            break;
        case SHELL_TYPE_VAR_POINT:
...
static int shellShowVar(Shell *shell, ShellCommand *command)
{
    ...
    case SHELL_TYPE_VAR_STRING:
        shellWriteString(shell, "\"");
        shellWriteString(shell, (char *)(unsigned long)value);
        shellWriteString(shell, "\"");
        break;
    // case SHELL_TYPE_VAR_INT:
    // case SHELL_TYPE_VAR_SHORT:
...
static void shellRunCommand(Shell *shell, ShellCommand *command)
{
    ...
    if (command->attr.attrs.type == SHELL_TYPE_CMD_MAIN)
    {
        ...
        {
            /* 只支持main类型的command打印返回信息 */
            shellWriteExitInfo(shell, command, returnValue);
    ...
    else if (command->attr.attrs.type == SHELL_TYPE_CMD_FUNC)
    {
        returnValue = shellExtRun(shell,
                                  ...
                                  shell->parser.param);
        shell->lastRet = returnValue; /* 记录上一条命令执行的返回值 */
        if (!command->attr.attrs.disableReturn)
        {
            shellWriteReturnValue(shell, returnValue);
        }
...
static void shellWriteReturnValue(Shell *shell, int value)
{
    if (shell->slient && (0 == value)) //skip returen value print if 'silent' and no error
        return;
 ...
 *
 * @param shell shell对象
 * @param ShellCommand command对象
 * @param value 返回值
 */
static void shellWriteExitInfo(Shell *shell, ShellCommand *command, int value)
{
    if ((NULL == shell) || (command == NULL))
        return;

    if (shell->slient && (0 == value)) //skip returen value print if 'silent' and no error
        return;

    const ShellExitInfo *exit_info = command->data.cmd.exit_info;
    if ((NULL == exit_info) || (SHELL_EXIT_CODE_MAGIC_NUM != exit_info->magic_num))
        return;

    unsigned int loop;
    const char *exit_msg = NULL;
    const ShellExitCode *exit_codes = exit_info->exit_codes;
    for (loop = 0; loop < exit_info->exit_code_num; loop++)
    {
        if (exit_codes[loop].exit_code == value)
        {
            exit_msg = exit_codes[loop].exit_msg;
        }
    }
    ...
    if (NULL != exit_msg)
    {
        shellWriteString(shell, exit_msg);
    }
    shellWriteString(shell, "] ");
    shellWriteString(shell, "\r\n");
}
...
/**
 * @name: shellCtrlc
 * @msg:  在shell命令函数中主动使用此接口，用于检测是否有ctrl + c 命令输入
 * @return {0} 没有检测到ctrl + c 命令，{1} 检测到ctrl + c 命令输入
 * @note: 使用此接口一般用于退出某些特定功能函数
 */
int shellCtrlc(void)
{
    char data;
