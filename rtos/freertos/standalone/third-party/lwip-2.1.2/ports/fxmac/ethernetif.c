...
#if LWIP_IGMP
static err_t xmac_filter_update (struct netif *netif,const ip4_addr_t *group, enum netif_mac_filter_action action);
...
static void UserConfigConvert(FXmacLwipPort * instance_p, UserConfig * config_p)
{
    ...
    FXmacLwipPortConfig mac_lwip_port_config;
...
static err_t low_level_init(struct netif *netif)
{
    ...
    instance_p = FXmacLwipPortGetInstancePointer(user_config->mac_instance);
    if(instance_p == NULL)
    {
    ...
    for (int i = 0; i < FXMAX_MAX_HARDWARE_ADDRESS_LENGTH; i++)
    {
    ...
    if(instance_p->feature & FXMAC_LWIP_PORT_CONFIG_UNICAST_ADDRESS_FILITER) 
    {
        LWIP_DEBUGF(NETIF_DEBUG, ("Set unicast hash table!!\n"));
        ret = FXmac_SetHash(&instance_p->instance, netif->hwaddr);
        if (ret != FT_SUCCESS)
        {
            FXMAC_LWIP_NET_PRINT_E("Set unicast hash table is error\r\n");
            mem_free(lwip_port);
            return ERR_ARG;
        }
    }
