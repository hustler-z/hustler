...
void FXmacLinkChange(void *args)
{
    ...
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("xmac_p->config.base_address is %p", xmac_p->config.base_address);
        ...
        {
        case 0:
            FXMAC_LWIP_PORT_XMAC_PRINT_I("link status is down");
            link_status = FXMAC_LINKDOWN;
            break;
        case 1:
            FXMAC_LWIP_PORT_XMAC_PRINT_I("link status is up");
...
static FError FXmacLwipPortConfigConvert(FXmacLwipPort *instance_p,FXmacConfig* xmac_config_p)
{
    ...
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_E("FXmacLookupConfig is error , instance_id is %d", instance_p->xmac_port_config.instance_id);
    ...
    case FXMAC_LWIP_PORT_INTERFACE_USXGMII:
        ...
        break;
    default:
        FXMAC_LWIP_PORT_XMAC_PRINT_E("update interface is error , interface is %d", instance_p->xmac_port_config.instance_id);
        return FREERTOS_XMAC_INIT_ERROR;
    }
    ...
    case FXMAC_PHY_SPEED_10G:
        ...
        break;
    default:
        FXMAC_LWIP_PORT_XMAC_PRINT_E("setting speed is not valid , speed is %d", instance_p->xmac_port_config.phy_speed);
        return FREERTOS_XMAC_INIT_ERROR;
    }
...
{
    u32 options=0;
    if (feature & FXMAC_LWIP_PORT_CONFIG_JUMBO)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("FXMAC_JUMBO_ENABLE_OPTION is ok");
        options |= FXMAC_JUMBO_ENABLE_OPTION;
    }
    
    if (feature & FXMAC_LWIP_PORT_CONFIG_UNICAST_ADDRESS_FILITER)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("FXMAC_UNICAST_OPTION is ok");
        options |= FXMAC_UNICAST_OPTION;
    }

    if (feature & FXMAC_LWIP_PORT_CONFIG_MULTICAST_ADDRESS_FILITER)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("FXMAC_MULTICAST_OPTION is ok");
        options |= FXMAC_MULTICAST_OPTION;
    }
    /* enable copy all frames */
    if (feature & FXMAC_LWIP_PORT_CONFIG_COPY_ALL_FRAMES)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("FXMAC_PROMISC_OPTION is ok");
        options |= FXMAC_PROMISC_OPTION;
    }
      /* close fcs check */
    if (feature & FXMAC_LWIP_PORT_CONFIG_CLOSE_FCS_CHECK)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_I("FXMAC_FCS_STRIP_OPTION is ok");
...
FError FXmacLwipPortInit(FXmacLwipPort *instance_p)
{
    ...
    status = FXmacLwipPortConfigConvert(instance_p,&mac_config);
    if (status != FT_SUCCESS)
    {
        FXMAC_LWIP_PORT_XMAC_PRINT_E("In %s: FXmacLwipPortControl Convert to FXmacConfig Failed....\r\n", __func__);
    }
    ...
    if (instance_p->feature & FXMAC_LWIP_PORT_CONFIG_RX_POLL_RECV)
    {
...
FXmacLwipPort * FXmacLwipPortGetInstancePointer(u32 FXmacLwipPortInstanceID)
{
    FASSERT(FXmacLwipPortInstanceID < FXMAC_NUM);
    
    FXmacLwipPort *instance_p;
    instance_p = &fxmac_lwip_port_instance[FXmacLwipPortInstanceID];
    return instance_p;
}
