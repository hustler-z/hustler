...
#ifndef _FSL_PARTITION_MBR_H
#define _FSL_PARTITION_MBR_H
...
struct dos_partition 
{
	...
	uint8_t bh, bs, bc;	/* begin CHS */
	uint8_t sys_ind;
	uint8_t eh, es, ec;	/* end CHS */
	uint8_t start_sect[4];
	uint8_t nr_sects[4];
} __attribute__((packed));

static inline unsigned int chs_get_cylinder(const uint8_t cylinder, const uint8_t sector) 
{
    return (cylinder + ((sector >> 6) << 8));
}

static inline unsigned int chs_get_sector(const uint8_t sector)
{
    return (sector & 0x3f);
}
...
static inline struct dos_partition *mbr_get_partition(uint8_t *mbr, int i)
{
...
static inline unsigned int __dos_assemble_4le(const uint8_t *p)
{
	return p[0] | (p[1] << 8) | (p[2] << 16) | (p[3] << 24);
}

static inline void __dos_store_4le(uint8_t *p, unsigned int val)
{
	p[0] = (val & 0xff);
	p[1] = ((val >> 8) & 0xff);
	p[2] = ((val >> 16) & 0xff);
...
static inline int mbr_is_valid_magic(const uint8_t *mbr)
{
	return mbr[510] == 0x55 && mbr[511] == 0xaa ? 1 : 0;
}

static inline void mbr_set_magic(uint8_t *b)
{
	b[510] = 0x55;
	b[511] = 0xaa;
}

static inline unsigned int mbr_get_id(const uint8_t *mbr)
{
	return __dos_assemble_4le(&mbr[440]);
}

static inline void mbr_set_id(uint8_t *b, unsigned int id)
{
