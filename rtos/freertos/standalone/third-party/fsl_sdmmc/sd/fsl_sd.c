...
{
    if (card->usrParam.ioVoltage->type == kSD_IOVoltageCtrlNotSupport)
    {
...
static status_t SD_SwitchVoltage(sd_card_t *card, sdmmc_operation_voltage_t voltage)
{
	 ...
	 */
    if (!SDMMCHOST_IsCardBusy(card->host))
    {
        ...
        if (!SDMMCHOST_IsCardBusy(card->host))
        {
            ...
            SDMMC_LOGE(TAG, "Card did not drive DAT lines low");
            return kStatus_SDMMC_CardStatusBusy;
        }
    }
	 ...
	 */
    (void)SDMMCHOST_SetCardClock(card->host, 0U);

    /* switch io voltage */
    if (SD_SwitchIOVoltage(card, voltage) == kStatus_SDMMC_NotSupportYet)
    {
        SDMMC_LOGE(TAG, "Failed to switch SD host to 1.8V");
    ...
    /* Restart the clock */
    (void)SDMMCHOST_SetCardClock(card->host, card->busClock_Hz);
    ...
    if (SDMMCHOST_IsCardBusy(card->host))
    {
        SDMMC_LOGE(TAG, "Card failed to switch voltages");
        return kStatus_SDMMC_SwitchVoltageFail;        
    }

    SDMMC_LOGD(TAG, "Card switched to 1.8V signaling");
    return error;
}
    ...
    do
    {
        ...
        if ((retry == 0U) || (error == kStatus_SDMMC_ReTuningRequest))
        {
            if ((card->currentTiming == kSD_TimingSDR50Mode) || (card->currentTiming == kSD_TimingSDR104Mode))
            {
                ...
                if (SD_ExecuteTuning(card) != kStatus_Success) /* CMD19 */
                {
...
static void SD_DecodeScr(sd_card_t *card, uint32_t *rawScr)
{
    ...
    SDMMC_LOGD(TAG, "SD reports specification version %d", card->version);
...
static status_t SD_SetDataBusWidth(sd_card_t *card, uint32_t width)
{
    ...
    if (kStatus_Success != SD_SendApplicationCmd(card, card->relativeAddress))
    {
        SDMMC_LOGE(TAG, "SD app command failed for ACMD6");
        return kStatus_SDMMC_SendApplicationCommandFailed;
    }
...
static void SD_DecodeCsd(sd_card_t *card, uint32_t *rawCsd)
{
    ...
    {
        SDMMC_LOGE(TAG, "unknown SD CSD structure version 0x%x", csd->csdStructure);
    ...
    SDMMC_LOGD(TAG, "Card block count %d, block size %d", card->blockCount, card->blockSize);
}
...
static status_t SD_SendCsd(sd_card_t *card)
{
    ...
    if (kStatus_Success == error)
    {
        ...
        SD_DecodeCsd(card, (uint32_t *)(uintptr_t)card->internalBuffer);
    }
    else
    {
...
static status_t SD_AllSendCid(sd_card_t *card)
{
    ...
    {
        (void)memcpy(card->internalBuffer, (uint8_t *)command.response, 16U);
        SD_DecodeCid(card, (uint32_t *)(uintptr_t)card->internalBuffer);
    ...
    while (0U != i--)
    {
        ...
        if ((command.response[0U] & SDMMC_MASK(kSD_OcrPowerUpBusyFlag)) != 0U)
        {
            ...
            if ((command.response[0U] & SDMMC_MASK(kSD_OcrCardCapacitySupportFlag)) != 0U)
            {
                ...
                SDMMC_LOGD(TAG, "Is high capcity card > 2GB");
            }
            /* 1.8V support */
            if ((command.response[0U] & SDMMC_MASK(kSD_OcrSwitch18AcceptFlag)) != 0U)
            {
                card->flags |= (uint32_t)kSD_SupportVoltage180v;
                SDMMC_LOGD(TAG, "Is UHS card support 1.8v");
            }
            else
            {
                SDMMC_LOGD(TAG, "Not UHS card only support 3.3v");
            }
    ...
    do
    {
        ...
        else
        {
            if ((command.response[0U] & 0xFFU) != 0xAAU)
            {
                ...
                SDMMC_LOG("\r\nError: card not support CMD8, resp: 0x%x\r\n", 
                         command.response[0U]);
            }
            else
            {
...
status_t SD_ReadStatus(sd_card_t *card)
{
    ...
    command.index        = (uint32_t)kSD_ApplicationStatus/*kSDMMC_SendStatus*/;
    command.responseType = kCARD_ResponseTypeR1;
...
static status_t SD_Read(sd_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockSize, uint32_t blockCount)
{
    ...
    data.rxData              = (uint32_t *)(uintptr_t)buffer;
                         ...
                         uint32_t *writtenBlocks)
{
    ...
    *writtenBlocks  = blockCount;
    data.blockCount = blockCount;
    data.txData     = (const uint32_t *)(uintptr_t)buffer;
...
status_t SD_ReadBlocks(sd_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockCount)
{
    ...
    while (blockLeft != 0U)
    {
        nextBuffer = (uint8_t *)((uintptr_t)buffer + blockDone * FSL_SDMMC_DEFAULT_BLOCK_SIZE);
        if ((!card->noInteralAlign) && (!dataAddrAlign || (!FSL_SDMMC_BUFFER_ALIGNED(nextBuffer))))
        {
            blockCountOneTime = 1;
...
status_t SD_WriteBlocks(sd_card_t *card, const uint8_t *buffer, uint32_t startBlock, uint32_t blockCount)
{
    ...
    while (blockLeft != 0U)
    {
        nextBuffer = (uint8_t *)((uintptr_t)buffer + (blockCount - blockLeft) * FSL_SDMMC_DEFAULT_BLOCK_SIZE);
        if (!card->noInteralAlign && (!dataAddrAlign || (!FSL_SDMMC_BUFFER_ALIGNED(nextBuffer))))
        {
            blockCountOneTime = 1;
...
static uint32_t SD_CalculateEraseTimeout(sd_card_t *card, uint32_t blockCount, uint32_t auSize)
{
    ...
    if ((card->stat.eraseTimeout != 0U) && (card->stat.eraseSize != 0U))
    {
        ...
        timeout_ms = auCount * (((uintptr_t)card->stat.eraseTimeout * 1000U) / (uintptr_t)card->stat.eraseSize + 500U) +
...
static status_t SD_ProbeBusVoltage(sd_card_t *card)
{
    ...
    uint32_t applicationCommand41Argument = 0U; /* OCR arguments */
    status_t error                        = kStatus_Success;
    ...
    {
        SDMMC_LOGD(TAG, "Support 1.8v (with SDR50/SDR104/DDR mode)");
    ...
    {
        /* card go idle */
        if (kStatus_Success != SD_GoIdle(card)) /* CMD0 */
        {
            error = kStatus_SDMMC_GoIdleFailed;
            break;
        }

        error = SD_SendInterfaceCondition(card); /* CMD8 */
        ...
        {
            /* SDSC card */
            if (kStatus_Success != SD_GoIdle(card)) /* make up for legacy card which do not support CMD8 */
            {
                error = kStatus_SDMMC_GoIdleFailed;
                break;
            }
        }
        error = SD_ApplicationSendOperationCondition(card, applicationCommand41Argument); /* ACMD41 */
...
static void SD_DumpCard(sd_card_t *card)
{
    char cardName[SD_PRODUCT_NAME_BYTES + 1] = {'\0'};

    memcpy((void *)cardName, (const void *)card->cid.productName, SD_PRODUCT_NAME_BYTES);
    cardName[SD_PRODUCT_NAME_BYTES] = '\0';
    SDMMC_LOG("Name: %s [@0x%x]\r\n", cardName, card->relativeAddress);

	switch (card->version)
	{
	case kSD_SpecificationVersion1_0:
		SDMMC_LOG("  Card-Version: SD 1.0");
		break;
	case kSD_SpecificationVersion1_1:
		SDMMC_LOG("  Card-Version: SD 1.1");
		break;
	case kSD_SpecificationVersion2_0:
		SDMMC_LOG("  Card-Version: SD 2.0");
		break;
	case kSD_SpecificationVersion3_0:
		SDMMC_LOG("  Card-Version: SD 3.0");
		break;
	default:
		break;
	}

    if (card->flags & kSD_SupportSdhcFlag)
        SDMMC_LOG(" SDHC ");

    if (card->flags & kSD_SupportSdxcFlag)
        SDMMC_LOG(" SDXC ");

    SDMMC_LOG("\r\n");

	SDMMC_LOG("  Size: %d GB\r\n", 
			  (uint32_t)(((uint64_t)card->blockSize * (uint64_t)card->blockCount) / (uint64_t)SZ_1G));

	if (card->busClock_Hz > (1000 * 1000)) {
		SDMMC_LOG("  Bus-Speed: %d MHz\r\n", card->busClock_Hz / (1000 * 1000));
	} else if (card->busClock_Hz > 1000) {
		SDMMC_LOG("  Bus-Speed: %d KHz\r\n", card->busClock_Hz / (1000));
	} else {
		SDMMC_LOG("  Bus-Speed: %d Hz\r\n", card->busClock_Hz);
	}
	
    switch (card->operationVoltage)
    {
        case kSDMMC_OperationVoltage330V:
            SDMMC_LOG("  Voltage: 3.3v\r\n");
            break;
        case kSDMMC_OperationVoltage300V:
            SDMMC_LOG("  Voltage: 3.0v\r\n");
            break;
        case kSDMMC_OperationVoltage180V:
            SDMMC_LOG("  Voltage: 1.8v\r\n");
            break;
        default:
            SDMMC_LOG("  Voltage: unkonwn\r\n");
            break;                        
    }
    ...
    switch (card->currentTiming)
    {
        case kSD_TimingSDR12DefaultMode:
            if (kSDMMC_OperationVoltage330V == card->operationVoltage)
                SDMMC_LOG("  Timing: Default-Speed\r\n");
            else if (kSDMMC_OperationVoltage180V == card->operationVoltage)
                SDMMC_LOG("  Timing: SDR12\r\n");
            break;
        case kSD_TimingSDR25HighSpeedMode:
            if (kSDMMC_OperationVoltage330V == card->operationVoltage)
                SDMMC_LOG("  Timing: High-Speed\r\n");
            else if (kSDMMC_OperationVoltage180V == card->operationVoltage)
                SDMMC_LOG("  Timing: SDR25\r\n");
            break;
        case kSD_TimingSDR50Mode:
            SDMMC_LOG("  Timing: SDR50\r\n");
            break;
        case kSD_TimingSDR104Mode:
            SDMMC_LOG("  Timing: SDR104\r\n");
            break;
        case kSD_TimingDDR50Mode:
            SDMMC_LOG("  Timing: DDR50\r\n");
            break;
        default:
            SDMMC_LOG("  Timing: Unknown\r\n");
            break;
    }

    switch (card->maxCurrent)
    {
    case kSD_CurrentLimit200MA:
        SDMMC_LOG("  Max. Current: 200mA\r\n");
        break;
    case kSD_CurrentLimit400MA:
        SDMMC_LOG("  Max. Current: 400mA\r\n");
        break;
    case kSD_CurrentLimit600MA:
        SDMMC_LOG("  Max. Current: 600mA\r\n");
        break;
    case kSD_CurrentLimit800MA:
        SDMMC_LOG("  Max. Current: 800mA\r\n");
        break;
    default:
        SDMMC_LOG("  Max. Current: unknown\r\n");
        break;
    }

    switch (card->driverStrength)
    {
    case kSD_DriverStrengthTypeA:
        SDMMC_LOG("  Drv. Type: A\r\n");
        break;
    case kSD_DriverStrengthTypeB:
        SDMMC_LOG("  Drv. Type: B\r\n");
        break;
    case kSD_DriverStrengthTypeC:
        SDMMC_LOG("  Drv. Type: C\r\n");
        break;
    case kSD_DriverStrengthTypeD:
        SDMMC_LOG("  Drv. Type: D\r\n");
        break;
    default:
        SDMMC_LOG("  Drv. Type: unknown\r\n");
        break;
    }
...
static status_t SD_CardInitProc(sd_card_t *card)
{
    assert(card != NULL);
    ...
    */
    ...
    error = SD_SelectCard(card, true); /* CMD7 */
    if (kStatus_Success != error)
    {
    ...
    */
    ...
    if ((card->flags & (uint32_t)kSD_Support4BitWidthFlag) != 0U)
    {
        ...
        error = SD_SetDataBusWidth(card, kSDMMC_BusWdith4Bit); /* ACMD6 */
        if (kStatus_Success != error)
        {
    ...
    error = SD_ReadStatus(card); /* ACMD13 */
    if (kStatus_Success != error)
    {
        return kStatus_SDMMC_SendScrFailed;
    }

    error = SD_SetBlockSize(card, card->blockSize/*FSL_SDMMC_DEFAULT_BLOCK_SIZE*/); /* CMD16 */
    /* set block size */
    if (kStatus_Success != error)
    {
...
status_t SD_HostInit(sd_card_t *card)
{
    ...
    if (!card->isHostReady)
    {
        ...
        if (error != kStatus_Success)
        {
            SDMMC_LOGE(TAG, "SD host driver init failed, error = %d", error);
            return kStatus_Fail;
        }
    }
    ...
    {
        SDMMC_LOGD(TAG, "SD card init start");
    ...
    SDMMC_LOGD(TAG, "SD host init success !!!");
    return kStatus_Success;
}
...
status_t SD_PollingCardInsert(sd_card_t *card, uint32_t status)
{
    ...
    else
    {
        ...
        if (card->isHostReady == false)
        {
            SDMMC_LOGE(TAG, "SD host not ready !!!");
            return kStatus_Fail;
        }
        ...
        {
            SDMMC_LOGE(TAG, "Polling SD card status failed !!!");
            return kStatus_Fail;
        }
    }
...
status_t SD_Init(sd_card_t *card)
{
    ...
    if (kStatus_Success == error)
    {
        ...
        {
            SDMMC_LOGE(TAG, "Polling card failed !!!");
            error = kStatus_SDMMC_CardDetectFailed;
        }
        else
        {
            /* start card init process */
            SDMMC_LOGI(TAG, "Start card identification");
            error = SD_CardInit(card);
            if (error != kStatus_Success)
            {
                SDMMC_LOGE(TAG, "SD card init failed !!!");
                error = kStatus_SDMMC_CardInitFailed;
            }
        }
    }

    SDMMC_LOGD(TAG, "SD init finished, error = %d", error);
    return error;
}
...
status_t FSDMMCHOST_CmdSwitch(sd_card_t *card)
{
    assert(card != NULL);
