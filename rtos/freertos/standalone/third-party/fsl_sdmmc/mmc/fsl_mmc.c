...
static status_t MMC_SetExtendedCsdConfig(mmc_card_t *card, const mmc_extended_csd_config_t *config, uint32_t timeout)
{
    ...
    /* CMD13 Wait for the card write process complete because of that card read process and write process use one buffer. */
...
static status_t MMC_SetMaxDataBusWidth(mmc_card_t *card, mmc_high_speed_timing_t targetTiming)
{
    ...
    do
    {
        ...
        if (card->busWidth == kMMC_DataBusWidth8bitDDR)
        {
            ...
            {
                SDMMC_LOGD(TAG, "Set buswidth 8-bit");
        ...
        if (card->busWidth == kMMC_DataBusWidth4bitDDR)
        {
            if (((card->host->capability &
                ...
                ((targetTiming == kMMC_HighSpeedTiming) || (targetTiming == kMMC_HighSpeed400Timing)))
            {
                SDMMC_LOGD(TAG, "Set buswidth 4-bit");
    ...
    if (error == kStatus_Fail)
    {
        ...
        SDMMC_LOGD(TAG, "Set buswidth 1-bit");
...
static status_t MMC_SwitchToHS400(mmc_card_t *card)
{
    ...
    if (card->host->sourceClock_Hz < MMC_CLOCK_HS400)
    {
        hs400Freq = card->host->sourceClock_Hz;
    }

    if ((card->host->sourceClock_Hz > MMC_CLOCK_HS400) &&
        (card->host->sourceClock_Hz % MMC_CLOCK_HS400 != 0U))
    {
        hs400Freq = card->host->sourceClock_Hz /
                    (card->host->sourceClock_Hz / MMC_CLOCK_HS400 + 1U);
    }
...
static status_t MMC_SelectBusTiming(mmc_card_t *card)
{
    ...
    do
    {
        ...
        if (card->busTiming == kMMC_HighSpeed400Timing)
        {
            ...
            {
                /* switch to HS400 */
                SDMMC_LOGD(TAG, "Set HS400 timing");
        ...
        if (card->busTiming == kMMC_HighSpeed200Timing)
        {
            ...
            {
                SDMMC_LOGD(TAG, "Set HS200 timing");
                if (kStatus_Success !=
        ...
        {
            SDMMC_LOGD(TAG, "Set Hight-speed timing");
...
static status_t MMC_SendCsd(mmc_card_t *card)
{
    ...
    if (kStatus_Success == error)
    {
        ...
        MMC_DecodeCsd(card, (uint32_t *)(uintptr_t)card->internalBuffer);
    ...
    mmc_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockSize, uint32_t blockCount)
{
    ...
    if (!card->noInteralAlign && (!FSL_SDMMC_BUFFER_ALIGNED(buffer)))
    {
        data.rxData              = (uint32_t *)(uintptr_t)alignBuffer;
    }
    else
    {
    ...
    {
        (void)memcpy(buffer, alignBuffer, blockSize * blockCount);
    }
    ...
    mmc_card_t *card, const uint8_t *buffer, uint32_t startBlock, uint32_t blockSize, uint32_t blockCount)
{
    ...
    if (!card->noInteralAlign && (!FSL_SDMMC_BUFFER_ALIGNED(buffer)))
    {
        (void)memcpy(alignBuffer, buffer, blockSize * blockCount);
        data.txData              = (const uint32_t *)(uintptr_t)alignBuffer;
    }
    else
    {
...
static void MMC_DumpCard(mmc_card_t *card)
{
    assert(card != NULL);
    char cardName[MMC_PRODUCT_NAME_BYTES + 1] = {'\0'};

    memcpy((void *)cardName, (const void *)card->cid.productName, MMC_PRODUCT_NAME_BYTES);
    cardName[MMC_PRODUCT_NAME_BYTES] = '\0';
    SDMMC_LOG("Name: %s [@0x%x]\r\n", cardName, card->relativeAddress);

	SDMMC_LOG("  Size: %d GB\r\n", 
			  (uint32_t)((((uint64_t)card->bootPartitionBlocks + (uint64_t)card->userPartitionBlocks)* 
                          (uint64_t)card->blockSize) / (uint64_t)SZ_1G));
    ...
    switch (card->busTiming)
    {
        ...
        case kMMC_HighSpeed200Timing:
            SDMMC_LOG("  Timing: HS200\r\n");
            break;
        case kMMC_HighSpeed400Timing:
            SDMMC_LOG("  Timing: HS400\r\n");
            break;
        case kMMC_EnhanceHighSpeed400Timing:
            SDMMC_LOG("  Timing: Enhanced HS400\r\n");
            break;
    }
    ...
    switch (card->busWidth)
    {
        case kMMC_DataBusWidth1bit:
            SDMMC_LOG("  Bus Width: 1-bit\r\n");
            break;
        case kMMC_DataBusWidth4bit:
            SDMMC_LOG("  Bus Width: 4-bit\r\n");
            break;            
        case kMMC_DataBusWidth8bit:
            SDMMC_LOG("  Bus Width: 8-bit\r\n");
            break;             
        case kMMC_DataBusWidth4bitDDR:
            SDMMC_LOG("  Bus Width: 4-bit DDR\r\n");
            break;                
        case kMMC_DataBusWidth8bitDDR:
            SDMMC_LOG("  Bus Width: 8-bit DDR\r\n");
            break;              
        case kMMC_DataBusWidth8bitDDRSTROBE:
            SDMMC_LOG("  Bus Width: 8-bit DDR STROBE\r\n");
            break;          
    }

    SDMMC_LOG("  Boot Partiion: %d\r\n", card->bootPartitionBlocks);
    SDMMC_LOG("  User Partiion: %d\r\n", card->userPartitionBlocks);
}

static status_t MMC_CardInitProc(mmc_card_t *card)
{
    assert(card != NULL);
    ...
    /* Hand-shaking with card to validata the voltage range Host first sending its expected
       information.*/
    error = MMC_SendOperationCondition(card, 0U); /* CMD1 */
    if (kStatus_Success != error)
    {
    ...
    error = MMC_SendOperationCondition(card, opcode); /* CMD1 */
    if (kStatus_Success != error)
    {
        return kStatus_SDMMC_HandShakeOperationConditionFailed;
    }
    ...
    /* Get card CID */
    error = MMC_AllSendCid(card); /* CMD2 */
    if (kStatus_Success != error)
    {
        return kStatus_SDMMC_AllSendCidFailed;
    }

    error = MMC_SetRelativeAddress(card); /* CMD3 */
    ...
    transfer state. */
    error = MMC_SelectCard(card, true); /* CMD7 */
    if (kStatus_Success != error)
    {
    ...
    error = MMC_SendExtendedCsd(card, NULL, 0U); /* CMD8 */
    if (kStatus_Success != error)
    {
        return kStatus_SDMMC_SendExtendedCsdFailed;
    }

    error = MMC_SetMaxEraseUnitSize(card); /* CMD6, CMD13 */
    ...
    error = MMC_SetBlockSize(card, FSL_SDMMC_DEFAULT_BLOCK_SIZE); /* CMD16 */
    /* set block size */
    if (kStatus_Success != error)
    {
    ...
    error = MMC_SelectBusTiming(card); /* CMD6 */
    ...
    error = MMC_SetPowerClass(card); /* CMD6 */
    /* switch power class */
    if (kStatus_Success != error)
    {
...
void MMC_HostReset(sdmmchost_t *host)
{
...
status_t MMC_ReadBlocks(mmc_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockCount)
{
    ...
    else
    {
        while (blockLeft != 0U)
        {
            ...
            if (!card->noInteralAlign && (!dataAddrAlign || ((((uintptr_t)nextBuffer) & (sizeof(uintptr_t) - 1U)) != 0U)))
            {
                blockLeft--;
...
status_t MMC_WriteBlocks(mmc_card_t *card, const uint8_t *buffer, uint32_t startBlock, uint32_t blockCount)
{
    ...
    else
    {
        ...
        {
            nextBuffer = (uint8_t *)((uintptr_t)buffer + blockDone * FSL_SDMMC_DEFAULT_BLOCK_SIZE);
            if (!card->noInteralAlign && (!dataAddrAlign || (0U != (((uintptr_t)nextBuffer) & (sizeof(uintptr_t) - 1U)))))
            {
                blockLeft--;
                blockCountOneTime = 1U;
