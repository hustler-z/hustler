...
#define SDMMC_LOGD( tag, format, ... ) FT_DEBUG_PRINT_D(tag, format, ##__VA_ARGS__)
#define SDMMC_LOGI( tag, format, ... ) FT_DEBUG_PRINT_I(tag, format,  ##__VA_ARGS__)
#define SDMMC_LOGW( tag, format, ... ) FT_DEBUG_PRINT_W(tag, format,  ##__VA_ARGS__)
#define SDMMC_LOGE( tag,  format, ... ) FT_DEBUG_PRINT_E(tag, format,  ##__VA_ARGS__)
#define SDMMC_LOGV( tag, format, ... ) FT_DEBUG_PRINT_V(tag, format,  ##__VA_ARGS__)
#define SDMMC_LOG(...)                 PORT_KPRINTF(__VA_ARGS__)
...
static inline uintptr_t __ALIGN(uintptr_t addr, uintptr_t aligment, size_t size, size_t *usage)
{
  uintptr_t mask = aligment - 1;
  uintptr_t start = addr;
  start += ((addr & mask) ? (aligment - (addr & mask)) : 0);
  if (usage)
    *usage = ((start + size) - addr);
  return start;
}
...
static inline unsigned int __FIND_MSB_SET(uint32_t op)
{
	if (op == 0) {
		return 0;
	}
