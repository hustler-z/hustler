...
static const char* TAG = "sdmmc_sdio";
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
/*!
...
static status_t SDIO_SendOperationCondition(sdio_card_t *card, uint32_t argument, uint32_t *accept1V8)
{
    ...
    while (--i != 0U)
    {
        ...
        {
            continue; /* card power-up is busy, try again */
        }
        else if (argument == 0U)
        {
    ...
    sdio_card_t *card, sdio_func_num_t func, uint32_t regAddr, uint8_t *buffer, uint32_t count, uint32_t flags)
{
    ...
    bool opCode                  = false;
    status_t error               = kStatus_Success;
    uint8_t *alignedBuffer       = (uint8_t *)FSL_SDMMC_CARD_INTERNAL_BUFFER_ALIGN_ADDR(card->internalBuffer);
    ...
    if (error == kStatus_Success)
    {
        ...
        if ((!card->noInternalAlign) && ((uintptr_t)buffer % data.blockSize))
        {
            /* use aligned buffer like SD, MMC, copy content to be write to that aligned buffer */
            memcpy((void *)alignedBuffer, (void *)buffer, data.blockSize * data.blockCount);
            data.txData = (uint32_t *)(uintptr_t)alignedBuffer;
        }
        else
        {
            data.txData = (uint32_t *)(uintptr_t)buffer;
        }
    ...
    sdio_card_t *card, sdio_func_num_t func, uint32_t regAddr, uint8_t *buffer, uint32_t count, uint32_t flags)
{
    ...
    if (error == kStatus_Success)
    {
        ...
        {
            /* use aligned buffer */
            data.rxData = (uint32_t *)(uintptr_t)alignedBuffer;
        }
        else
        {
        ...
        {
            if ((uintptr_t)data.rxData != (uintptr_t)buffer)
            {
                /* copy content just read from aligned buffer to user input buffer */
                memcpy((void *)buffer, (void *)data.rxData, (data.blockSize * data.blockCount));
            }
        }
    }
                          ...
                          uint32_t *response)
{
    ...
    sdmmchost_data_t data        = {0U};
    uint32_t i                   = SDIO_RETRY_TIMES;
    uint32_t *dataAddr           = (uint32_t *)(uintptr_t)(txData == NULL ? rxData : txData);
    ...
    else
    {
        ...
        if (dataSize != 0U)
        {
            ...
            if (
                ...
                (((uintptr_t)dataAddr & (SDMMCHOST_DMA_DESCRIPTOR_BUFFER_ALIGN_SIZE - 1U)) != 0U) &&
#endif
                (actualSize <= FSL_SDMMC_DEFAULT_BLOCK_SIZE) && (!card->noInternalAlign))
            {
                dataAddr = (uint32_t *)(uintptr_t)alignBuffer;
...
#if SDMMCHOST_DMA_DESCRIPTOR_BUFFER_ALIGN_SIZE != 1U
                    (((uintptr_t)rxData & (SDMMCHOST_DMA_DESCRIPTOR_BUFFER_ALIGN_SIZE - 1U)) != 0U) &&
#endif
                    (actualSize <= FSL_SDMMC_DEFAULT_BLOCK_SIZE) && (!card->noInternalAlign))
                {
...
status_t SDIO_GetCardCapability(sdio_card_t *card, sdio_func_num_t func)
{
    ...
    /*
      FBR (function 1), 0x100 ~ 0x1ff
      ...
      FBR (function 7), 0x700 ~ 0x7ff
    */
    ...
    switch (func)
    {
        case kSDIO_FunctionNum0:
            ...
            card->sdVersion    = tempBuffer[kSDIO_RegSDVersion]; /* 0x1, SD Specification Revision */
            card->sdioVersion  = tempBuffer[kSDIO_RegCCCRSdioVer] >> 4U; /* 0x0, SDIO Revision */
            card->cccrVersioin = tempBuffer[kSDIO_RegCCCRSdioVer] & 0xFU; /* 0x0, CCCR Revision */
            /* continuous SPI interrupt */
            if ((tempBuffer[kSDIO_RegBusInterface] & 0x40U) != 0U) /* 0x7, Bus Interface Control */
            {
                card->cccrflags |= (uint32_t)kSDIO_CCCRSupportContinuousSPIInt; /* 1, SCSI */
            }
            /* 8bit data bus */
            if ((tempBuffer[kSDIO_RegBusInterface] & 0x4U) != 0U)
            {
                card->cccrflags |= SDIO_CCCR_SUPPORT_8BIT_BUS; /* 1, 8-bit support */
            }

            /* card capability register */
            card->cccrflags |= (tempBuffer[kSDIO_RegCardCapability] & 0xDFUL); /* 0x8, Card Capability */
            /* master power control */
            if ((tempBuffer[kSDIO_RegPowerControl] & 0x01U) != 0U) /* 0x12, Power Control */
            {
                card->cccrflags |= (uint32_t)kSDIO_CCCRSupportMasterPowerControl; /* 1, SMPC */
            }
            /* high speed flag */
            if ((tempBuffer[kSDIO_RegBusSpeed] & 0x01U) != 0U) /* 0x13, Bus Speed Select */
            {
                card->cccrflags |= SDIO_CCCR_SUPPORT_HIGHSPEED; /* 1, SHS */
            }
            /* uhs mode flag */
            card->cccrflags |= (tempBuffer[kSDIO_RegUHSITimingSupport] & 7UL) << 11U; /* 0x14, UHS-I Support */
            /* driver type flag */
            card->cccrflags |= (tempBuffer[kSDIO_RegDriverStrength] & 7UL) << 14U; /* 0x15, Driver Strength */
            /* low speed 4bit */
            if ((tempBuffer[kSDIO_RegCardCapability] & 0x80U) != 0U) /* 0x8, Card Capability */
            {
                card->cccrflags |= (uint32_t)kSDIO_CCCRSupportLowSpeed4Bit; /* 1, 4BLS */
            }
            /* common CIS pointer */
            card->commonCISPointer = tempBuffer[kSDIO_RegCommonCISPointer] | /* 0x9 ~ 0xb, Common CIS Pointer */
            ...
            if ((tempBuffer[kSDIO_RegInterruptExtension] & SDIO_CCCR_ASYNC_INT_MASK) == SDIO_CCCR_ASYNC_INT_MASK) /* 0x16, Interrupt Extension */
            {
...
status_t SDIO_ReadCIS(sdio_card_t *card, sdio_func_num_t func, const uint32_t *tupleList, uint32_t tupleNum)
{
    ...
    uint8_t *dataBuffer = SDMMC_OSAMemoryAllocate(255U);
    if (NULL == dataBuffer)
    {
    ...
    if (0U == cisPtr)
    {
        SDMMC_OSAMemoryFree(dataBuffer);
        return kStatus_SDMMC_SDIO_ReadCISFail;
    }
...
static status_t SDIO_ProbeBusVoltage(sdio_card_t *card)
{
    ...
    do
    {
        ...
        /* Get IO OCR-CMD5 with arg0 ,set new voltage if needed, CMD5 */
        if (kStatus_Success != SDIO_SendOperationCondition(card, 0U, NULL)) /* OCR = 0, just probing, do a single pass */
        {
...
static void SDIO_DumpCard(sdio_card_t *card)
{
    SDMMC_LOG("Manufacturer: 0x%x-0x%x-0x%x\r\n", 
              card->commonCIS.mID, 
              card->commonCIS.mInfo,
              card->commonCIS.funcID);

    switch (card->sdVersion)
    {
        case 0x0:
            SDMMC_LOG(" SD 1.0");
            break;
    ...
    SDMMC_LOG(" Block size: %d\r\n", card->commonCIS.fn0MaxBlkSize);          
    SDMMC_LOG(" Max speed: %ld MHz\r\n", card->commonCIS.maxTransSpeed);
    SDMMC_LOG(" IO function num: %d\r\n", card->ioTotalNumber);
}

static status_t SDIO_CardInitProc(sdio_card_t *card)
{
    assert(card != NULL);
    ...
    /* get card capability (CCCR), CMD52 */
