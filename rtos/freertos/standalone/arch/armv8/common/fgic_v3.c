 ...
 */
FError __attribute__((optimize("O0"))) FGicRedistrubutiorInit(FGic *instance_p)
{
    FASSERT(instance_p);
    FASSERT(instance_p->is_ready == FT_COMPONENT_IS_READY);
    s32 int_id ;
...
/**
 * @name: uintptr FGicRedistrubutiorIterate(void)
 * @msg:  Get gic redistrubutior base address of current core
 * @param {void}  
...
uintptr FGicRedistrubutiorIterate(void)
{
    ...
    {
        gicr_typer_aff = FGIC_GICR_TYPER_H_READ(redis_base);
 ...
 * @param {uintptr} redis_base is the redistributor address of current core
 * @return {*}
 */
FError FGicCfgInitialize(FGic *instance_p, const FGicConfig *input_config_p, uintptr redis_base)
{
    ...
    FASSERT(instance_p && input_config_p && redis_base);
