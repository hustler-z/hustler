 ...
 *
 * 16 Private Peripheral Interrupts (PPIs), that are independent for each core and can
 * be programmed to support either edge-triggered or level-sensitive interrupts.
 *
 * 16 SGIs, that are generated either by using software to write to GICD_SGIR or
 * through the GIC CPU interface of a core.
 *
 * Interrupt Translation Service (ITS). This provides device isolation and ID translation
 * for message-based interrupts, which allows virtual machines to program devices directly
