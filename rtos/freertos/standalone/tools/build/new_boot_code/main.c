...
#define IMAGE_DELAY_MAX 1000000000
...
static FError LoadCpu(void)
{
    ...
    for (int i = 0; i< FCORE_NUM; i++)
    {
        if(((1<<i) & cpu_core_mask) && (i != cpu_id))
        {
            ...
            if (image_info.startup_delay_ms < IMAGE_DELAY_MAX)
            {
                fsleep_millisec(image_info.startup_delay_ms);
            }
            else
            {
                printf(" Image delay (ms) max : %d. ",IMAGE_DELAY_MAX);
                fsleep_millisec(IMAGE_DELAY_MAX);
            }
        }
    }
