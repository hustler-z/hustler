 ...
 * LPIs are new in GICv3, and they are different to the other
 * types of interrupt in a number of ways.
 * In particular, LPIs are always message-based interrupts,
 * and their configuration is held in tables in memory rather
 * than registers.
 * --------------------------------------------------------------
 *
 * XXX: INTIDs
