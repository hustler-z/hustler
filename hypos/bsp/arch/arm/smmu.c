 ...
 * File:  smmu.c
 * Date:  2024/07/19
 * Usage: System Management Unit (a.k.a IOMMU)
 */
...
#define dev_dbg(dev, fmt, ...)			\
	MSGI("SMMUv3: %s: " fmt, dev_name(dev), ## __VA_ARGS__)
				...
				paddr_t *dma_handle, gfp_t gfp)
{
	...
	if (size & (size - 1)) {
		MSGI("SMMUv3: Fixing alignment for the DMA buffer\n");
		alignment = sizeof(void *);
	}

	vaddr = _xzalloc(size, alignment);
	if (!vaddr) {
		MSGI("SMMUv3: DMA allocation failed\n");
		return NULL;
	}
				       ...
				       struct arm_smmu_master *master)
{
	...
	MSGI("SMMUv3: d%u: vmid 0x%x vtcr 0x%"PRIpaddr" p2maddr 0x%"PRIpaddr"\n",
...
static int arm_smmu_add_device(u8 devfn, struct device *dev)
{
	...
	master = zalloc(struct arm_smmu_master);
	if (!master)
		return -ENOMEM;
...
{
	if (smmu->cmdq.q.base)
		free(smmu->cmdq.q.base);

	if (smmu->evtq.q.base)
		free(smmu->evtq.q.base);

	if (smmu->priq.q.base)
		free(smmu->priq.q.base);

	if (smmu->strtab_cfg.strtab)
		free(smmu->strtab_cfg.strtab);

	if (smmu->strtab_cfg.l1_desc)
		free(smmu->strtab_cfg.l1_desc);
}
...
static int arm_smmu_device_probe(struct platform_device *pdev)
{
	...
	smmu = zalloc(struct arm_smmu_device);
	if (!smmu)
		return -ENOMEM;
	smmu->dev = pdev;
