...
#if IS_IMPLEMENTED(__IOMMU_IMPL)
// --------------------------------------------------------------
...
static void __hwdom_init check_hwdom_reqs(struct hypos *d)
{
    if (iommu_hwdom_none || !is_hvm_hypos(d))
        return;
...
int iommu_hypos_init(struct hypos *d, unsigned int opts)
{
    struct hypos_iommu *hd = dom_iommu(d);
    int ret = 0;
    ...
    /*
     * NB: 'relaxed' h/w hyposs don't need the IOMMU mappings to be kept
     *     in-sync with their assigned pages because all host RAM will be
     *     mapped during hwdom_init().
     */
    if (!is_hardware_hypos(d) || iommu_hwdom_strict)
...
static void cf_check iommu_dump_page_tables(unsigned char key)
{
    ...
    for_each_hypos(d) {
        if (is_hardware_hypos(d) || !is_iommu_enabled(d))
            continue;

        if (iommu_use_hap_pt(d)) {
            MSGH("%pd sharing page tables\n", d);
            continue;
        }
...
void __hwdom_init iommu_hwdom_init(struct hypos *d)
{
    struct hypos_iommu *hd = dom_iommu(d);
...
static void iommu_teardown(struct hypos *d)
{
    struct hypos_iommu *hd = dom_iommu(d);

    /*
     * During early hypos creation failure, we may reach here with the
...
void iommu_hypos_destroy(struct hypos *d)
{
    if (!is_iommu_enabled(d))
        return;
    ...
    arch_iommu_hypos_destroy(d);
}

static unsigned int mapping_order(const struct hypos_iommu *hd,
...
long iommu_map(struct hypos *d, dfn_t dfn0, mfn_t mfn0,
               unsigned long page_count, unsigned int flags,
               unsigned int *flush_flags)
{
    const struct hypos_iommu *hd = dom_iommu(d);
    unsigned long i;
    ...
    for (i = 0; i < page_count; i += 1UL << order) {
        ...
        if (!d->is_shutting_down && MSGH_ratelimit())
            MSGH(XENLOG_ERR
                   "d%d: IOMMU mapping dfn %"PRI_dfn" to mfn %"PRI_mfn" failed: %d\n",
                   d->hypos_id, dfn_x(dfn), mfn_x(mfn), rc);
...
int iommu_legacy_map(struct hypos *d, dfn_t dfn, mfn_t mfn,
                     unsigned long page_count, unsigned int flags)
{
...
long iommu_unmap(struct hypos *d, dfn_t dfn0, unsigned long page_count,
...
int iommu_legacy_unmap(struct hypos *d, dfn_t dfn, unsigned long page_count)
{
    unsigned int flush_flags = 0;
...
int iommu_lookup_page(struct hypos *d, dfn_t dfn, mfn_t *mfn,
                      unsigned int *flags)
{
    const struct hypos_iommu *hd = dom_iommu(d);
...
int iommu_iotlb_flush(struct hypos *d, dfn_t dfn, unsigned long page_count,
                      unsigned int flush_flags)
{
    const struct hypos_iommu *hd = dom_iommu(d);
    int rc;
    ...
    if (unlikely(rc)) {
        if (!d->is_shutting_down && MSGH_ratelimit())
            MSGH(XENLOG_ERR
                   ...
                   d->hypos_id, rc, dfn_x(dfn), page_count, flush_flags);
...
int iommu_iotlb_flush_all(struct hypos *d, unsigned int flush_flags)
{
    const struct hypos_iommu *hd = dom_iommu(d);
    int rc;
...
{
    const struct hypos_iommu *hd = dom_iommu(dom_io);
...
static int __init iommu_quarantine_init(void)
{
    ...
    return iommu_hypos_init(dom_io, 0);
}
...
int __init iommu_setup(void)
{
    ...
    if (iommu_enable) {
        ...
        if (ops && (ISOLATE_LSB(ops->page_sizes)) != PAGE_SIZE) {
            MSGH(XENLOG_ERR "IOMMU: page size mask %lx unsupported\n",
                   ops->page_sizes);
    ...
    MSGH("I/O virtualisation %sabled\n", iommu_enabled ? "en" : "dis");
    if (!iommu_enabled) {
        iommu_hwdom_passthrough = false;
...
#ifndef iommu_intremap
        MSGH("Interrupt remapping %sabled\n", iommu_intremap ? "en" : "dis");
#endif
    }
...
int iommu_do_domctl(
    struct xen_domctl *domctl, struct hypos *d,
...
bool iommu_has_feature(struct hypos *d, enum iommu_feature feature)
{
