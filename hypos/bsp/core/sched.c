...
static void stimer_fn(void *unused);
...
DEFINE_PERCPU(atomic_t, sched_urgent_count);
...
static void sched_idle_schedule(
    const struct scheduler *ops, struct sched_unit *unit, stime_t now,
...
static inline void vcpu_runstate_change(
    struct vcpu *v, int new_state, stime_t new_entry_time)
{
    stime_t delta;
    ...
    for_each_domain_in_cpupool ( d, c )
    {
        ...
        for_each_sched_unit ( d, unit )
        {
                 ...
                 cpumask_test_cpu(cpu, unit->cpu_hard_affinity) )
            {
                ...
                MSGH(XENLOG_DEBUG "Breaking affinity for %pv\n",
                       unit->vcpu_list);
...
long do_set_timer_op(stime_t timeout)
{
    struct vcpu *v = current;
 ...
 */
void vcpu_set_periodic_timer(struct vcpu *v, stime_t value)
{
    spin_lock(&v->periodic_timer_lock);
...
static struct sched_unit *do_schedule(struct sched_unit *prev, stime_t now,
                                      unsigned int cpu)
{
    ...
    if ( prev->next_time >= 0 ) /* -ve means no limit */
        set_timer(&sr->stimer, now + prev->next_time);
...
static void sched_context_switch(struct vcpu *vprev, struct vcpu *vnext,
                                 bool reset_idle_unit, stime_t now)
{
...
static void sched_slave(void)
{
    ...
    stop_timer(&get_sched_res(cpu)->stimer);
...
static int cpu_schedule_up(unsigned int cpu)
{
    ...
    init_timer(&sr->stimer, stimer_fn, NULL, cpu);
...
#define sched_test_func(f)                               \
        if ( !schedulers[i]->f )                         \
        {                                                \
            MSGH("scheduler %s misses .%s, dropped\n", \
...
#undef sched_test_func
        ...
        {
            MSGH("scheduler %s failed initialization, dropped\n",
    ...
    if ( !scheduler )
    {
        MSGH("Could not find scheduler: %s\n", opt_sched);
    ...
    {
        MSGH("WARNING: sched_ratelimit_us outside of valid range [%d,%d].\n"
...
int schedule_cpu_rm(unsigned int cpu, struct cpu_rm_data *data)
{
    ...
    for_each_cpu ( cpu_iter, sr->cpus )
    {
        ...
        else
        {
            ...
            /* Init timer. */
            init_timer(&data->sr[idx]->stimer, stimer_fn, NULL, cpu_iter);
...
#endif
// --------------------------------------------------------------
void __bootfunc vcpu_sched_setup(void)
{
