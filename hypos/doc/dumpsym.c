 ...
 *
 * Usage: nm -n hypos.elf | scripts/dumpsym [--all-symbols] > symbols.S
 */
...
static unsigned long __text_start, __text_end, __boot_start, __boot_end;
...
{
    fprintf(stderr, "Usage: dumpsym [--all-symbols] [--symbol-prefix=<prefix char>] < in.map > out.S\n");
    exit(1);
}
...
static int read_symbol(FILE *in, struct sym_entry *s)
{
    ...
    if (strcmp(sym, "__text_start") == 0)
        __text_start = s->addr;
    else if (strcmp(sym, "__text_end") == 0)
        __text_end = s->addr;
    else if (strcmp(sym, "__boot_start") == 0)
        __boot_start = s->addr;
    else if (strcmp(sym, "__boot_end") == 0)
        __boot_end = s->addr;
...
static int symbol_valid(struct sym_entry *s)
{
    ...
    if (!all_symbols) {
        if ((s->addr < __text_start || s->addr > __text_end)
            && (s->addr < __boot_start || s->addr > __boot_end))
            return 0;
        /* Corner case.  Discard any symbols with the same value as
         * __text_end __boot_end; they can move between pass
         * 1 and 2 when the symbols data are added.  If these symbols
         * move then they may get dropped in pass 2, which breaks the
         * symbols rules.
         */
        if ((s->addr == __text_end && strcmp((char*)s->sym + offset, "__text_end")) ||
            (s->addr == __boot_end && strcmp((char*)s->sym + offset, "__boot_end")))
            return 0;
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2) {
        for (i = 1; i < argc; i++) {
            ...
            } else if (strcmp(argv[i], "--sysv") == 0) /* Not Used by this Project */
