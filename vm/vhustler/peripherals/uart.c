...
{
    /* Disable the UART */
    serial_dev->CR &= ~CR_UARTEN;

    /* Enable FIFOs */
    serial_dev->LCRH |= LCRH_FEN;

    // Interrupt mask register
    serial_dev->IMSC |= IMSC_RXIM;

    // Interrupt clear register
    serial_dev->ICR  = BE_INTERRUPT;
...
void uart_print_register_colon(const char *s, uint64_t n,
    const char *s1, uint64_t n1)
{
...
{
    uint32_t status = serial_dev->MIS;

    if (status & RX_INTERRUPT) {
        char c = serial_dev->DR & DR_DATA_MASK;
        uart_print_char(c);
        uart_print_char('\n');
    }
}
